...
const DOC_TYPES = [
  ...
  { k: 'akt',         label: 'Акт приёма-передачи', blurb: 'Передача имущества/работ',      active: true },
...
const INTAKE_GREETING = {
  ...
  akt: 'Опишите: кто передаёт и кто принимает, что именно передаётся (имущество, результат работ, ключи, документы), в каком состоянии и когда. Если чего-то не хватит — я уточню.',
...
const DOC_EXAMPLES = {
  ...
  akt: 'Акт приёма-передачи квартиры. Иванов И.И. (арендодатель) передаёт Петрову П.П. (арендатор) квартиру по ул. Токтогула 12, кв. 5, состояние хорошее, показания счётчиков — электричество 1234, вода 567.',
...
const getDocSnapshot=()=>{
  if (window.__ooMode) {
    const t = window.__ooDocText || '';
    const s = window.__ooSelection || '';
    return { text: t, selection: s, hasSelection: !!s };
  }
...
const App=()=>{
  ...
  useEffect(()=>{
    ...
    // Загрузить текст документа из бэкенда → window.__ooDocText (для getDocSnapshot)
    window.__ooLoadDocText = (fileId) => {
      if (!fileId) return;
      window.__ooFileId = fileId;
      fetch(OO_BACKEND + '/api/files/' + fileId + '/text')
        .then(r => r.json())
    ...
    return () => {
      ...
      delete window.__ooMode; delete window.__ooBridge; delete window.__ooOpenDocx;
      delete window.__ooLoadDocText; delete window.__ooFileId;
      delete window.__ooDocText; delete window.__ooSelection;
    };
  }, []);
  ...
  useEffect(()=>{
    ...
    fetch(OO_BACKEND + '/api/files/upload', { method: 'POST', body: fd })
      .then(r => r.json())
      .then(({ fileId }) => { if (fileId) { setTabFileIds(p => ({ ...p, [activeTab]: fileId })); if (window.__ooLoadDocText) window.__ooLoadDocText(fileId); } })
