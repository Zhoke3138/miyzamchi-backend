...
console.log('=== TEST 1: SEARCH_TOOL и DI guards ===');
{
    ...
    // 2026-06-02 (anti-conflict): старый "АЛГОРИТМ ПРИ ОШИБКАХ ЦИТИРОВАНИЯ" удалён,
    // потому что пункт 4 прямо приказывал указывать на ошибку, что конфликтовало
    // с Правилом №3 (презумпция правоты). Единственный источник истины — IRON RULES.
    assert(!TOOL_PROTOCOL_BLOCK.includes('АЛГОРИТМ ПРИ ОШИБКАХ ЦИТИРОВАНИЯ'),
        '🛑 Удалён конфликтующий блок "АЛГОРИТМ ПРИ ОШИБКАХ ЦИТИРОВАНИЯ" (был пункт 4 — конфликт с Правилом №3)');
    assert(!TOOL_PROTOCOL_BLOCK.includes('ЗАПРЕТ НА УГАДАЙКУ'),
        '🛑 Старый заголовок "ЗАПРЕТ НА УГАДАЙКУ" удалён (его суть теперь в Правиле №3)');
    // 2026-06-02: запрет на стилистические придирки внутри Правила №1
    assert(TOOL_PROTOCOL_BLOCK.includes('советы по стилистике') &&
           TOOL_PROTOCOL_BLOCK.includes('названия конвенций к каждому пункту'),
        '🛑 Правило №1 включает запрет на стилистические замечания (вкусовщина ≠ юр.риск)');
