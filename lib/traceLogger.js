...
// ── Листинг для GET /api/traces (Debug-архив) ───────────────────────────
// Возвращает массив { fileName, modifiedAt, modifiedAtIso, sizeKB, sizeBytes, url },
// отсортированный по mtime (новые сверху). Только валидные `trace_*.md` файлы.
async function listTraces() {
    let files = [];
    try {
        files = await fs.promises.readdir(TRACES_DIR);
    } catch (e) {
        if (e.code === 'ENOENT') return [];
        throw e;
    }
    const valid = files.filter(_isFilenameSafe);
    const entries = await Promise.all(valid.map(async (fileName) => {
        try {
            const st = await fs.promises.stat(path.join(TRACES_DIR, fileName));
            return {
                fileName,
                ...
                url: `/api/trace/${encodeURIComponent(fileName)}`
            };
        } catch (_) {
            return null;
        }
    }));
    return entries.filter(Boolean).sort((a, b) => b.modifiedAt - a.modifiedAt);
}
