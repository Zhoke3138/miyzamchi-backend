...
async function classifyBlocks(chunks, deps = {}) {
    ...
    // ⚡ ОПТИМИЗАЦИЯ: батчи запускаем параллельно (Promise.all).
    // Разные батчи пишут в разные диапазоны labels[i] → конфликтов нет.
    // Для 19 блоков: 2 батча последовательно = ~4-6с → параллельно = ~2-3с.
    const batchStarts = [];
    for (let start = 0; start < n; start += CLASSIFY_BATCH) batchStarts.push(start);

    await Promise.all(batchStarts.map(async (start) => {
