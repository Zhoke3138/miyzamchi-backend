    ...
    async function runSimplePath(chunkAnalyses, telemetry, aborted) {
        ...
        const results = await runWithConcurrency(queue, SIMPLE_PINECONE_CONCURRENCY, async (item) => {
            ...
            try {
                ...
                const matches = await searchPinecone(vector, 1);
                if (telemetry?.incrementCounter) telemetry.incrementCounter('pinecone_simple_queries');
    ...
    async function runHeavyForChunk(ca, telemetry, res, aborted, degradedState) {
        ...
        const pass1 = await runWithConcurrency(ca.citations, SIMPLE_PINECONE_CONCURRENCY, async (citation) => {
            ...
            try {
                ...
                const vector = await getEmbedding(query);
                if (!vector) return [];
                const matches = await searchPinecone(vector, HEAVY_PASS1_TOP_K);
                if (telemetry?.incrementCounter) telemetry.incrementCounter('pinecone_heavy_pass1_queries');
                return matches || [];
            } catch (e) {
