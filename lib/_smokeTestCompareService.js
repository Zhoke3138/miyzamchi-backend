...
// ── 6. legalAudit: короткий якорь → skipped (без сети) ───────────────
(async () => {
  const short = await svc.legalAudit({ oldText: 'г. Бишкек, 2026 г.', newText: 'г. Бишкек, 2026 г.' });
  ok('legalAudit: короткий пункт → skipped', short.status === 'skipped');

  const oldClause = 'Исполнитель несёт ответственность за нарушение сроков поставки тепловой энергии в размере 0,1% за каждый день просрочки, но не более 10% от стоимости.';
  const newClause = 'Исполнитель полностью освобождается от любой ответственности за нарушение сроков поставки тепловой энергии при любых обстоятельствах.';

  // ── 7. КИЛЛЕР-ФИКС: ЯКОРНЫЙ поиск идёт по СТАРОМУ тексту ───────────
  let seenQueryFor = null;
  const anchorDeps = {
    expandQuery: async (txt) => { seenQueryFor = txt; return { npa: null, article: null, queries: ['q'] }; },
    pineconeSearch: async () => ([{ id: '1', score: 0.9, metadata: { npa_title: 'Правила теплоснабжения', full_text: 'норма' } }]),
    validate: async () => ({ status: 'error', marker: '🔴 ОШИБКА', detail: 'нарушает императивную норму', cited_articles: ['ГК КР, ст.310'] }),
  };
  const viol = await svc.legalAudit(
    { oldText: oldClause, newText: newClause, subject: 'теплоснабжение', governingNpas: ['Правила теплоснабжения'] },
    anchorDeps
  );
  ok('legalAudit: поиск по СТАРОМУ тексту (anchor=oldText)', seenQueryFor === oldClause);
  ok('legalAudit: вернул error', viol.status === 'error');
  ok('legalAudit: прокинул cited_articles', viol.cited_articles[0] === 'ГК КР, ст.310');
  ok('legalAudit: effectiveNpa = документ-уровневый', viol.npa === 'Правила теплоснабжения');

  // ── 8. legalAudit: добавленный пункт (нет oldText) → якорь = newText ─
  let seenAdded = null;
  const addedDeps = {
    expandQuery: async (txt) => { seenAdded = txt; return { npa: null, article: null, queries: ['q'] }; },
    pineconeSearch: async () => ([]),
    validate: async () => { throw new Error('не должно вызываться'); },
  };
  const added = await svc.legalAudit({ oldText: '', newText: newClause, subject: 'теплоснабжение' }, addedDeps);
  ok('legalAudit: добавление → якорь newText', seenAdded === newClause);
  ok('legalAudit: пустой Pinecone → no_base', added.status === 'no_base');

  // ── 9. legalAudit: сбой deps → graceful skipped ───────────────────
  const boomDeps = { expandQuery: async () => { throw new Error('boom'); } };
  const boom = await svc.legalAudit({ oldText: oldClause, newText: newClause }, boomDeps);
