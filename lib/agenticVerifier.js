...
У тебя есть функция search_legislation_kg(query, reason). Используй её 1-2 раза, чтобы получить из базы НПА КР до 5 норм, релевантных проверяемому пункту. Запросы формулируй СЕМАНТИЧЕСКИ + С НАЗВАНИЕМ АКТА И СТРАНЫ (см. Правило №2 ниже). Например: "ГК КР неустойка в договоре оказания услуг" — это даёт точные попадания в правильную юрисдикцию. Без названия акта поиск часто притаскивает статьи с тем же номером из другой отрасли.

⚠️ ВАЖНО — Sniper RAG 3.0: каждая возвращённая «статья» — это ЧАСТЬ или ПУНКТ статьи (не вся статья). Поля ответа:
  • «npa»      — название НПА (Трудовой кодекс КР, ГК КР и т.д.)
  • «npa_type» — тип НПА (кодекс/закон/постановление/правила — иерархия юридической силы)
  • «context»  — позиция в иерархии: "РАЗДЕЛ > Глава > Статья X > Часть Y" — ОТСюда берёшь номер части/статьи для цитирования
  • «article»  — название статьи (может совпадать с context)
  • «text»     — сам текст части/пункта — ЭТО источник истины для вердикта
  Если несколько чанков одной статьи → синтезируй в единую оценку, не дублируй.
  При коллизии норм (разные «npa») → «npa_type» кодекс > закон > подзаконный акт (Lex Superior).
...
Оценивай текст пользователя опираясь на Паспорт документа и отрасль права из Паспорта. ОБЩИЕ принципы отрасли (структуру кодексов, типовые конструкции договоров и т.д.) можешь использовать, но КОНКРЕТНЫЕ номера и тексты статей — ТОЛЬКО из выдачи search_legislation_kg. RAG-выдача — единственный источник истины по точным нормам. Не вспоминай статьи из весов модели (см. Правило №3).
...
⛔ ТЕБЕ СТРОГО ЗАПРЕЩЕНО давать советы по стилистике и требовать от автора дописывать названия конвенций к каждому пункту списка, если контекст раздела и так очевиден. Если из заголовка раздела или предыдущих абзацев ясно, какой акт обсуждается (например, "Нарушения МПГПП" + далее идёт список "ст.7, ст.9, ст.14"), НЕ пиши "автору следует указать полное название Пакта при каждом упоминании". Это вкусовщина, а не юридический риск.
...
function _normArticle(c) {
    ...
    return {
        ...
        npa_type:       String(md.npa_type        || ''),   // Sniper RAG 3.0: кодекс/закон/правила/…
        article_title:  String(md.article_title   || ''),
        parent_context: String(md.parent_context  || md.article_title || ''),  // полная цепочка иерархии
        full_text:      full,
...
function createAgenticVerifier(deps = {}) {
    ...
    async function _runGeminiTier(tierCfg, opts) {
        ...
        // 2026-06-02: убрали logVerifierSystemPrompt/UserPrompt — раздували
        // trace.md повторами статичного блока правил + passport/topology.
        // Оставлены только variable per-chunk данные: start, tool_call/tool_response,
        // final_text, verdict. tracePromptsLogged-флаг сохраняется в сигнатуре
        // для обратной совместимости вызовов из routes/analyze.js, но фактически
        // не используется (legacy guard, можно удалить вместе с параметром при
        // следующем рефакторе сигнатуры).
        if (tracePromptsLogged && !tracePromptsLogged.done) {
        ...
        for (let turn = 0; turn < maxToolTurns; turn++) {
            ...
            if (functionCall) {
                ...
                // Исполняем поиск через Supabase с HCR-обогащённым embedding.
                ...
                try {
                    ...
                    candidates2 = await searchPinecone(vector, query, topK);
                } catch (e) {
                    pineconeErr = e;
                    logger.warn?.(`[AgenticVerifier] db search failed: ${e.message}`);
                }
                ...
                if (pineconeErr) {
                    ...
                    responsePayload = {
                        ...
                        articles: normalized.map((a, i) => ({
                            index:    i + 1,
                            npa:      a.npa_title,
                            npa_type: a.npa_type || undefined,        // Sniper RAG 3.0: кодекс/закон/…
                            context:  a.parent_context || undefined,  // Sniper RAG 3.0: полная цепочка иерархии
                            article:  a.article_title,
                            text:     a.full_text,
                            similarity: a.similarity
                        })),
                        instructions: 'КРИТИЧНО оцени каждую статью: релевантна (из ожидаемых НПА Паспорта) или false positive (другая отрасль). Если несколько чанков одной статьи — синтезируй. Для цитирования используй «context» (точная позиция в иерархии) и «text» (текст части/пункта). Игнорируй нерелевантные. Верни финальный JSON-вердикт.'
                    };
                }
            ...
            if (toolCallsArr.length > 0) {
                ...
                for (const tc of toolCallsArr) {
                    ...
                    try {
                        ...
                        pineconeErr = e;
                        logger.warn?.(`[AgenticVerifier/DeepSeek] db search failed: ${e.message}`);
                    }
                    if (telemetry?.recordAgentTime) {
                        telemetry.recordAgentTime('pineconeSearch', performance.now() - pineconeStart);
                    }
                    ...
                    if (pineconeErr) {
                        ...
                        toolContent = {
                            ...
                            articles: normalized.map((a, i) => ({
                                index:    i + 1,
                                npa:      a.npa_title,
                                npa_type: a.npa_type || undefined,
                                context:  a.parent_context || undefined,
                                article:  a.article_title,
                                text:     a.full_text,
                                similarity: a.similarity
                            })),
    ...
    async function _runDeepSeekLegacy(tierCfg, opts) {
        ...
        try {
            if (targetType === 'multi-article' && Array.isArray(articleGroup) && articleGroup.length > 0) {
                const groups = await Promise.all(articleGroup.map(async (art) => {
                    ...
                    return searchPinecone(v, q, Math.min(3, topK));
                }));
                ...
                const v = await getEmbedding(buildHCREmbeddingQuery(q, passport, topology));
                allCandidates = await searchPinecone(v, q, topK);
            } else {
                const q = textHead || userPrompt.slice(0, 300);
                const v = await getEmbedding(buildHCREmbeddingQuery(q, passport, topology));
                allCandidates = await searchPinecone(v, q, topK);
            }
        } catch (e) {
        ...
        const ragContext = articles.length
            ? articles.map((a, i) => {
                const typeTag = a.npa_type ? ` | ${a.npa_type}` : '';
                return `[${i + 1}] ${a.npa_title}${typeTag}\nКонтекст нормы: ${a.parent_context}\nТекст нормы: ${a.full_text}`;
              }).join('\n\n')
