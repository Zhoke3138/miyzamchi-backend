...
// 🟢 Защита от зависаний API (ВОЗВРАЩЕНО)
...
async function extractTextFromMedia(mimeType, base64Data) {
    ...
    while (retries <= maxRetries) {
        ...
        try {
            ...
            // 🟢 ИСПОЛЬЗУЕМ ТАЙМАУТ ИЗ ЛОКАЛЬНОЙ ВЕРСИИ
    ...
    try {
        ...
        while (retries <= maxRetries) {
            ...
            try {
                ...
                // 🟢 ВОЗВРАЩАЕМ ЗАЩИТУ ТАЙМАУТА
                const resultStream = await withTimeout(chat.sendMessageStream(promptText), 20000);
                ...
                const ttftSec = (t_first_token - t_start) / 1000;
                const genSec = (t_end - t_first_token) / 1000;
                const cps = genSec > 0 ? Math.round(fullText.length / genSec) : 0;
                
                const totalSec = ((performance.now() - tTotal0) / 1000).toFixed(2);
                console.log(`[⏱️ X-RAY Telegram] Total: ${totalSec}s | Embed: ${(tEmbed / 1000).toFixed(2)}s | DB: ${(tDB / 1000).toFixed(2)}s | TTFT: ${ttftSec.toFixed(2)}s | Gen: ${genSec.toFixed(2)}s (${cps} cps) | Retries: ${retries}`);
                ...
                if (retries >= maxRetries) {
                    const totalSec = ((performance.now() - tTotal0) / 1000).toFixed(2);
                    console.log(`[⏱️ X-RAY Telegram] FAILED Total: ${totalSec}s | Embed: ${(tEmbed / 1000).toFixed(2)}s | DB: ${(tDB / 1000).toFixed(2)}s | TTFT: n/a | Gen: n/a | Retries: ${retries}`);
        ...
        const totalSec = ((performance.now() - tTotal0) / 1000).toFixed(2);
        console.log(`[⏱️ X-RAY Telegram] ERROR Total: ${totalSec}s | Embed: ${(tEmbed / 1000).toFixed(2)}s | DB: ${(tDB / 1000).toFixed(2)}s | TTFT: n/a | Gen: n/a | Retries: n/a`);
