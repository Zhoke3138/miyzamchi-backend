...
- **Бэкенд** (Node/Express) — `server.js` + `routes/` + `services/` + `lib/`. Деплой на **Render** (`npm start` → `node server.js`). *(Парсинг — локально в Node; микросервис `parser-service/` Docling@CloudRun УДАЛЁН 11.06.2026, Этап 1 Backend Pivot.)*
...
- **Песочницы-кладбища**: `ide/`, `ide_bak/`, `ide-vite/` — **УДАЛЕНЫ** (10.06.2026, коммит cleanup). Осталось `scratch/` — черновики, игнорировать.
...
| Сервисы | [services/](services/) | `parserService.js` (ЛОКАЛЬНЫЙ парсинг: pdf-parse/mammoth/fs), `llmClients.js`, `legalAgents.js`, `compareService.js` |
| Парсинг (в services/) | `parserService.js` | Локально: текстовый PDF→pdf-parse (≤8МБ), DOCX→mammoth, TXT→fs. Тяжёлые/сканированные PDF→**Gemini Vision** (File API, модель `GEMINI_VISION_MODEL`/gemini-2.5-flash). ~~parser-service/ Docling@CloudRun~~ снесён 11.06.2026. |
...
### Роуты статики
`server.js` отдаёт `GET /` → `index.html` и статику из `public/`. Легаси-роут
`/ide` (старый TipTap-IDE) **удалён** 10.06.2026 вместе с папкой `ide/`.
...
> ❗ **`dispatchSuperDocTool` / Split Execution в активном коде НЕ используется.** `/api/edit` переписан на `{reasoning, commands}` (как handleAgent), `executeAIEdit` применяет команды через `applyAgentCommand` (нативный `editor.doc`). Старый headless-SDK путь удалён.

---

## 🗑️ Deprecated / Sandbox (ИГНОРИРОВАТЬ при написании кода)

Эти директории/файлы НЕ собираются root-Vite, НЕ импортируются `src/`. **Не редактировать, не брать за образец.**

> 🧹 **Удалены 10.06.2026** (chore cleanup): `ide/` (легаси TipTap-IDE на Babel-standalone), `ide_bak/` (бэкап), `ide-vite/` (песочница миграции TipTap→SuperDoc со скриптами-костылями). Активный фронт — только `src/`. Роут `/ide` тоже убран из server.js.
