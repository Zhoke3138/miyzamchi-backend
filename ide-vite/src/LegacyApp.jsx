...
/* ═══════════ EDITOR ADAPTER (works with Quill OR TipTap based on USE_TIPTAP) ═══════════ */
const editorAdapter = {
  isReady: () => USE_TIPTAP ? !!window.docEngine : !!window.currentQuill,
  getHTML: () => {
    if (USE_TIPTAP) return window.docEngine ? window.docEngine.getHTML() : '';
    return window.currentQuill ? window.currentQuill.root.innerHTML : '';
  },
  setHTML: (html) => {
    const safeHtml = html || '<p></p>';
    if (USE_TIPTAP) {
      if (window.docEngine) window.docEngine.commands.setContent(safeHtml, true);
    } else {
      if (window.currentQuill) window.currentQuill.root.innerHTML = safeHtml;
    }
  },
  getText: () => {
    if (USE_TIPTAP) return window.docEngine ? window.docEngine.getText() : '';
    return window.currentQuill ? window.currentQuill.getText() : '';
  },
  getSelectionInfo: () => {
    if (USE_TIPTAP) {
      const e = window.docEngine; if (!e) return null;
      const { from, to } = e.state.selection;
      if (from === to) return { hasSelection: false, text: '' };
      return { hasSelection: true, text: e.state.doc.textBetween(from, to, '\n'), from, to };
    } else {
      const q = window.currentQuill; if (!q) return null;
      const sel = q.getSelection();
      if (!sel || sel.length === 0) return { hasSelection: false, text: '' };
      return { hasSelection: true, text: q.getText(sel.index, sel.length), index: sel.index, length: sel.length };
    }
  },
  insertAtCursor: (text) => {
    if (USE_TIPTAP) {
      const e = window.docEngine; if (!e) return;
      e.chain().focus().insertContent(text).run();
    } else {
      const q = window.currentQuill; if (!q) return;
      const sel = q.getSelection();
      const idx = sel ? sel.index : Math.max(0, q.getLength() - 1);
  ...
  insertAtEnd: (text) => {
    if (USE_TIPTAP) {
      ...
      e.chain().focus().insertContentAt(end, '<p>' + text.replace(/\n/g, '</p><p>') + '</p>').run();
    } else {
      const q = window.currentQuill; if (!q) return;
      const idx = Math.max(0, q.getLength() - 1);
      q.insertText(idx, '\n' + text + '\n');
      q.setSelection(idx + text.length + 2, 0);
    }
  },
  replaceSelection: (text) => {
    if (USE_TIPTAP) {
      const e = window.docEngine; if (!e) return false;
      const { from, to } = e.state.selection;
      if (from === to) return false;
      e.chain().focus().deleteSelection().insertContent(text).run();
      return true;
    } else {
      const q = window.currentQuill; if (!q) return false;
      const sel = q.getSelection();
      if (!sel || sel.length === 0) return false;
      q.deleteText(sel.index, sel.length);
      q.insertText(sel.index, text);
      q.setSelection(sel.index + text.length, 0);
      return true;
    }
  },
  replaceAll: (text) => {
    if (USE_TIPTAP) {
      ...
      e.commands.setContent(html, true);
    } else {
      const q = window.currentQuill; if (!q) return;
      q.setText(text + '\n');
      q.setSelection(0, 0);
  ...
  insertSmart: (text, anchor) => {
    if (!anchor || anchor.length < 3) {
      editorAdapter.insertAtEnd(text);
      return false; // Returns false to indicate fallback
    }
    if (USE_TIPTAP) {
      const e = window.docEngine; if (!e) return false;
      let targetPos = -1;
      const lowerAnchor = anchor.toLowerCase().trim();
      ...
      e.state.doc.descendants((node, pos) => {
        ...
        if (node.isText) {
          ...
          if (idx !== -1) {
            targetPos = pos + idx + anchor.length;
          }
        }
      });
      
      if (targetPos !== -1) {
        e.chain().focus().insertContentAt(targetPos, '<p>' + text.replace(/\n/g, '</p><p>') + '</p>').run();
        return true;
      } else {
        editorAdapter.insertAtEnd(text);
        return false;
      ...
      const q = window.currentQuill; if (!q) return false;
      const fullText = q.getText().toLowerCase();
      const lowerAnchor = anchor.toLowerCase().trim();
      const idx = fullText.indexOf(lowerAnchor);
      if (idx !== -1) {
        const insertPos = idx + anchor.length;
        q.insertText(insertPos, '\n' + text + '\n');
        q.setSelection(insertPos + text.length + 2, 0);
        return true;
      } else {
        editorAdapter.insertAtEnd(text);
        return false;
...
const insertIntoQuill=(text,toastFn)=>{
  if(!editorAdapter.isReady()){toastFn&&toastFn('warning','Редактор не найден');return;}
  editorAdapter.insertAtEnd(text);
  toastFn&&toastFn('check','Вставлено в редактор');
};

/* ═══════════ AGENT MODE HELPERS ═══════════ */
const getDocSnapshot=()=>{
  if(!editorAdapter.isReady()) return {text:'',selection:'',hasSelection:false};
  const text=editorAdapter.getText()||'';
  const sel=editorAdapter.getSelectionInfo();
  return {text:text.trim(),selection:(sel?.text||'').trim(),hasSelection:!!sel?.hasSelection};
};
...
const applyAgentCommand=(cmd,toastFn)=>{
  if(!editorAdapter.isReady()){toastFn&&toastFn('warning','Редактор не найден');return false;}
  const text=String(cmd.text||'').trim();
  if(!text){toastFn&&toastFn('warning','Пустой текст команды');return false;}
  try{
    if(cmd.type==='insert_smart'){
      const ok = editorAdapter.insertSmart(text, cmd.anchor);
      if(ok){
      ...
      editorAdapter.insertAtCursor(text);
      toastFn&&toastFn('check','Вставлено');
    }else if(cmd.type==='replace_selection'){
      const ok=editorAdapter.replaceSelection(text);
...
const convertDocxToHtml = async (buffer) => {
  // window.docx in MIyzamchy = docx-preview (UMD), works for renderAsync
  const dp = window.docxPreview || window.docx;
  if (dp?.renderAsync) {
    const stage = document.createElement('div');
    stage.style.cssText = 'position:absolute;left:-99999px;top:0;width:794px;visibility:hidden;pointer-events:none;';
    document.body.appendChild(stage);
    try {
      await dp.renderAsync(buffer, stage, null, {
        inWrapper: true, ignoreFonts: false, ignoreWidth: false, ignoreHeight: true,
        ignoreLastRenderedPageBreak: true, breakPages: false, useBase64URL: true,
        experimental: false, renderHeaders: false, renderFooters: false,
        renderFootnotes: false, renderEndnotes: false,
      });
      const html = cleanDocxPreviewDom(stage);
      return html || '<p></p>';
    } finally {
  ...
  if (window.mammoth) {
    ...
    return res.value || '<p></p>';
  }
  throw new Error('Ни docx-preview, ни mammoth не загружены');
};
...
const AIEditorTipTap = ({ onToast, onCtx }) => {
  const elRef = useRef(null);
  const editorRef = useRef(null);
  const canvasRef = useRef(null);
  // Refs для callbacks — чтобы useEffect [] не пересоздавал редактор на каждом re-render
  const onToastRef = useRef(onToast);
  const onCtxRef = useRef(onCtx);
  useEffect(() => { onToastRef.current = onToast; onCtxRef.current = onCtx; });
  const [editorInst, setEditorInst] = useState(null);
  const [ready, setReady] = useState(!!window.TipTap);
  const [loadError, setLoadError] = useState(window.__tiptapError || null);
  const [pageCount, setPageCount] = useState(1);
  const [wordCount, setWordCount] = useState(0);
  const [zoom, setZoom] = useState(() => {
    const saved = parseFloat(localStorage.getItem(ZOOM_KEY));
    return ZOOM_PRESETS.includes(saved) ? saved : 1;
  });
  const [marginLeft, setMarginLeft] = useState(() => parseFloat(localStorage.getItem('docengine-ml')) || PAD_LEFT_MM_DOC);
  const [marginRight, setMarginRight] = useState(() => parseFloat(localStorage.getItem('docengine-mr')) || PAD_RIGHT_MM_DOC);

  useEffect(() => {
    document.documentElement.style.setProperty('--doc-pad-left', marginLeft + 'mm');
    localStorage.setItem('docengine-ml', String(marginLeft));
  }, [marginLeft]);
  useEffect(() => {
    document.documentElement.style.setProperty('--doc-pad-right', marginRight + 'mm');
    localStorage.setItem('docengine-mr', String(marginRight));
  }, [marginRight]);
  useEffect(() => {
  ...
  useEffect(() => {
    document.documentElement.style.setProperty('--doc-pages', String(pageCount));
  }, [pageCount]);
  ...
  useEffect(() => {
    ...
    const initEditor = () => {
      if (disposed) return;
      if (!window.TipTap || !elRef.current || editorRef.current) return;
      const T = window.TipTap;
      try {
        const editor = new T.Editor({
          element: elRef.current,
          extensions: [
            // StarterKit v3 уже включает Link и Underline — отключаем чтобы не было дубликатов
            T.StarterKit.configure({ heading: { levels: [1, 2, 3, 4] }, link: false, underline: false }),
            T.Underline,
            T.TextAlign.configure({ types: ['heading', 'paragraph'] }),
            T.TextStyle, T.FontFamily, T.Color,
            ...
            T.Table.configure({ resizable: true, HTMLAttributes: { class: 'docengine-table' } }),
            T.TableRow, T.TableHeader, T.TableCell,
            T.Placeholder.configure({ placeholder: 'Начните печатать… или откройте файл слева.' }),
            T.CharacterCount,
            T.FontSize, T.LineHeight, T.TextIndent,
            T.AgentPending,
          ],
          content: '<p></p>',
          autofocus: 'end',
          editorProps: {
            attributes: { class: 'docengine-prose', spellcheck: 'true' },
            transformPastedHTML: (html) => {
              if (!html) return html;
              const isWord = new RegExp('(<o:p|class="?Mso|mso-|<w:|<m:)', 'i').test(html);
              ...
              const rules = [
                ...
                [new RegExp('\\s+(class|lang|align)="?Mso[^"\\s>]*"?', 'gi'), ''],
                [new RegExp('<p([^>]*)>(\\s|&nbsp;)*</p>', 'gi'), '<p$1></p>'],
              ];
              let cleaned = html;
              for (const [re, rep] of rules) cleaned = cleaned.replace(re, rep);
              return cleaned;
            },
          },
        });
        editorRef.current = editor;
        window.docEngine = editor;
        editor.view.dom.addEventListener('contextmenu', (e) => {
          if (!onCtxRef.current) return;
          e.preventDefault();
          onCtxRef.current(e.clientX, e.clientY, [{ label: 'Улучшить (A4 wired)', icon: 'sparkles', action: () => {} }]);
        });
        console.log('[MZ] AIEditorTipTap (full) mounted');
        setReady(true);
        setEditorInst(editor);
      } catch (e) {
        console.error('[MZ] AIEditorTipTap init failed:', e);
        setLoadError(e.message || String(e));
        if (onToastRef.current) onToastRef.current('warning', 'TipTap init fail: ' + e.message);
      }
    };
    if (window.TipTap) initEditor();
    else {
      const onR = () => initEditor();
      const onE = (ev) => setLoadError(ev.detail || 'unknown');
      window.addEventListener('tiptap-ready', onR, { once: true });
      window.addEventListener('tiptap-error', onE, { once: true });
      return () => { disposed = true; window.removeEventListener('tiptap-ready', onR); window.removeEventListener('tiptap-error', onE); };
    }
    return () => {
      disposed = true;
      if (editorRef.current) { editorRef.current.destroy(); editorRef.current = null; window.docEngine = null; }
    };
  }, []);

  /* ═══ AGENT INLINE PREVIEW (Cursor-style) ═══ */
  const [pendingEdit, setPendingEdit] = useState(null);
  const [pendingRect, setPendingRect] = useState(null);
  const pendingEditRef = useRef(null);
  useEffect(() => { pendingEditRef.current = pendingEdit; }, [pendingEdit]);

  const recomputePendingRect = useCallback(() => {
    const e = editorRef.current;
    const pe = pendingEditRef.current;
    if (!e || !pe || !pe.range) { setPendingRect(null); return; }
    try {
      // Position floating bar at end of pending range
      const coords = e.view.coordsAtPos(Math.min(pe.range.to, e.state.doc.content.size));
      const left = (coords.left + coords.right) / 2;
      const top = coords.bottom + 6;
      const visTop = Math.max(60, Math.min(window.innerHeight - 60, top));
      const visLeft = Math.max(140, Math.min(window.innerWidth - 140, left));
      setPendingRect({ left: visLeft, top: visTop });
    } catch (err) { setPendingRect(null); }
  }, []);

  useEffect(() => {
    if (!pendingEdit) { setPendingRect(null); return; }
    recomputePendingRect();
    const onScroll = () => recomputePendingRect();
    const onResize = () => recomputePendingRect();
  ...
  }, [pendingEdit, recomputePendingRect]);
  ...
  const acceptPendingEdit = useCallback(() => {
    ...
    try {
      ...
      div.innerHTML = html;
      div.querySelectorAll('.agent-pending-delete').forEach(el => el.remove());
      div.querySelectorAll('.agent-pending-insert').forEach(el => {
        while (el.firstChild) el.parentNode.insertBefore(el.firstChild, el);
        el.remove();
      });
      e.commands.setContent(div.innerHTML, true);
    } catch (err) { console.error('accept failed', err); }
    setPendingEdit(null);
    if (onToastRef.current) onToastRef.current('check', 'Применено');
    if (pe.onResolve) pe.onResolve('applied');
  }, []);
  ...
  const rejectPendingEdit = useCallback(() => {
    ...
    if (!e || !pe) return;
    try {
      if (pe.original && pe.original.html) {
        e.commands.setContent(pe.original.html, true);
      }
    } catch (err) { console.error('reject failed', err); }
    setPendingEdit(null);
    if (onToastRef.current) onToastRef.current('x', 'Отклонено');
    if (pe.onResolve) pe.onResolve('rejected');
  }, []);

  // ════════════════════════════════════════════════════════════════
  // FORMATTING INHERITANCE — capture style at anchor, apply to insert
  // Чтобы вставленный текст наследовал шрифт/размер/выравнивание
  // окружающего параграфа (как Cursor сохраняет indentation).
  // ════════════════════════════════════════════════════════════════
  const captureFormattingAt = (editor, pos) => {
    try {
      const safePos = Math.max(0, Math.min(pos, editor.state.doc.content.size));
      const $pos = editor.state.doc.resolve(safePos);
      // depth=0 is doc, depth=1 is usually paragraph/heading
      const paragraph = $pos.depth > 0 ? $pos.node($pos.depth) : $pos.parent;
      const paraAttrs = paragraph?.attrs || {};
      let textStyle = {};
      // Walk children of the paragraph to find the most recent textStyle marks
      if (paragraph && paragraph.forEach) {
        paragraph.forEach((child) => {
          if (child.isText && child.marks) {
            const ts = child.marks.find(m => m.type.name === 'textStyle');
            if (ts) textStyle = { ...textStyle, ...ts.attrs };
          }
        });
      }
      return {
        textStyle,
        paraAttrs: {
          textAlign: paraAttrs.textAlign,
          lineHeight: paraAttrs.lineHeight,
          textIndent: paraAttrs.textIndent,
          marginLeft: paraAttrs.marginLeft,
          marginRight: paraAttrs.marginRight,
        }
      };
    } catch (err) {
      console.warn('[Agent] captureFormattingAt failed:', err.message);
      return { textStyle: {}, paraAttrs: {} };
    }
  };
  const applyFormattingToRange = (editor, range, formatting) => {
    try {
      const { textStyle = {}, paraAttrs = {} } = formatting || {};
      // Apply text-level marks (font-family, size, color)
      const tChain = editor.chain().setTextSelection(range);
      let any = false;
      if (textStyle.fontFamily && tChain.setFontFamily) { tChain.setFontFamily(textStyle.fontFamily); any = true; }
      if (textStyle.fontSize && tChain.setFontSize)     { tChain.setFontSize(textStyle.fontSize); any = true; }
      if (textStyle.color && tChain.setColor)           { tChain.setColor(textStyle.color); any = true; }
      if (any) tChain.run();
      // Apply paragraph-level attrs
      const pChain = editor.chain().setTextSelection(range);
      let pAny = false;
      if (paraAttrs.textAlign && pChain.setTextAlign)   { pChain.setTextAlign(paraAttrs.textAlign); pAny = true; }
      if (paraAttrs.lineHeight && pChain.setLineHeight) { pChain.setLineHeight(paraAttrs.lineHeight); pAny = true; }
      if (paraAttrs.textIndent && pChain.setTextIndent) { pChain.setTextIndent(paraAttrs.textIndent); pAny = true; }
      if (paraAttrs.marginLeft && pChain.setMarginLeft) { pChain.setMarginLeft(paraAttrs.marginLeft); pAny = true; }
      if (paraAttrs.marginRight && pChain.setMarginRight) { pChain.setMarginRight(paraAttrs.marginRight); pAny = true; }
      if (pAny) pChain.run();
    } catch (err) {
      console.warn('[Agent] applyFormattingToRange failed:', err.message);
    }
  };

  // Expose API to chat / agent system
  useEffect(() => {
    window.previewAgentEdit = (cmd, opts) => {
      const e = editorRef.current;
      if (!e) { if (onToastRef.current) onToastRef.current('warning', 'Редактор не готов'); return false; }
      if (pendingEditRef.current) rejectPendingEdit();
      const text = String(cmd.text || '').trim();
      if (!text) { if (onToastRef.current) onToastRef.current('warning', 'Пустой текст'); return false; }
      let range = null, original = null;
      try {
        original = { html: e.getHTML() };
        if (cmd.type === 'insert_smart') {
          // Find anchor in document text-nodes via ProseMirror descendants.
          // Always operate on doc positions (not plain-text indexOf — those differ).
          const anchor = String(cmd.anchor || '').trim();
          const html = '<p>' + text.split(/\n\n+/).map(p => p.replace(/\n/g, '<br>')).join('</p><p>') + '</p>';
          let targetPos = -1;
          if (anchor && anchor.length >= 3 && anchor !== 'EMPTY') {
            const lowerAnchor = anchor.toLowerCase();
            e.state.doc.descendants((node, pos) => {
              if (targetPos !== -1) return false;
              if (node.isText && node.text) {
                const idx = node.text.toLowerCase().indexOf(lowerAnchor);
                if (idx !== -1) {
                  // Insert position = end of anchor inside this text node
                  targetPos = pos + idx + anchor.length;
                }
              }
            });
          }
          // Capture formatting BEFORE insertion (otherwise positions shift)
          const insertPos = targetPos !== -1 ? targetPos : e.state.doc.content.size;
          const formatting = captureFormattingAt(e, Math.max(0, insertPos - 1));
          if (targetPos === -1 && anchor && anchor !== 'EMPTY') {
            console.warn('[Agent] Anchor not found, falling back to insert_end. Anchor:', anchor);
            if (onToastRef.current) onToastRef.current('warning', 'Якорь не найден — вставлено в конец');
          }
          // Insert HTML at position
          e.chain().focus().insertContentAt(insertPos, html).run();
          const after = e.state.selection.from;
          range = { from: insertPos, to: after };
          // Inherit formatting (font, size, color, paragraph attrs)
          applyFormattingToRange(e, range, formatting);
          // Apply pending-edit mark for diff highlight
          e.chain().setTextSelection(range).setMark('agentPending', { kind: 'insert' }).run();
        } else if (cmd.type === 'insert_end') {
          const before = e.state.doc.content.size;
          const html = '<p>' + text.split(/\n\n+/).map(p => p.replace(/\n/g, '<br>')).join('</p><p>') + '</p>';
          // Capture formatting from the last text in the document
          const formatting = captureFormattingAt(e, Math.max(0, before - 1));
          e.chain().focus().insertContentAt(before, html).run();
          const after = e.state.doc.content.size;
          range = { from: before, to: after };
          applyFormattingToRange(e, range, formatting);
          e.chain().setTextSelection(range).setMark('agentPending', { kind: 'insert' }).run();
        } else if (cmd.type === 'insert_cursor') {
          const before = e.state.selection.from;
          // Capture formatting at cursor — TipTap insertContent inherits cursor marks naturally,
          // but we re-apply paragraph attrs (textAlign, lineHeight) which marks don't carry.
          const formatting = captureFormattingAt(e, Math.max(0, before - 1));
          e.chain().focus().insertContent(text).run();
          const after = e.state.selection.from;
          range = { from: before, to: after };
          applyFormattingToRange(e, range, formatting);
          e.chain().setTextSelection(range).setMark('agentPending', { kind: 'insert' }).run();
        } else if (cmd.type === 'replace_selection' || cmd.type === 'replace_all') {
          let oldText = '';
          let fromPos = 0;
          if (cmd.type === 'replace_selection') {
            const sel = e.state.selection;
            if (sel.from === sel.to) { if (onToastRef.current) onToastRef.current('warning', 'Нет выделения для замены'); return false; }
            oldText = e.state.doc.textBetween(sel.from, sel.to, '\n');
            fromPos = sel.from;
          } else {
            oldText = e.getText();
            fromPos = 0;
          ...
          if (window.diff_match_patch) {
            ...
            const diffs = dmp.diff_main(oldText, text);
            dmp.diff_cleanupSemantic(diffs);
            outHtml = '<p>';
            diffs.forEach(([op, part]) => {
              const lines = part.split('\n');
              lines.forEach((line, i) => {
                if (i > 0) outHtml += '</p><p>';
                if (line) {
                  const safePart = line.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
                  if (op === 1) outHtml += `<span class="agent-pending-insert">${safePart}</span>`;
                  else if (op === -1) outHtml += `<span class="agent-pending-delete">${safePart}</span>`;
                  else outHtml += safePart;
                }
              });
            });
            outHtml += '</p>';
            outHtml = outHtml.replace(/<p><\/p>/g, '<p><br></p>');
          } else {
            outHtml = '<p>' + text.split(/\n\n+/).map(p => p.replace(/\n/g, '<br>')).join('</p><p>') + '</p>';
            outHtml = `<span class="agent-pending-insert">${outHtml}</span>`;
          }

          if (cmd.type === 'replace_selection') {
            e.chain().deleteSelection().insertContentAt(fromPos, outHtml).run();
            range = { from: fromPos, to: e.state.selection.to };
          } else {
            e.commands.setContent(outHtml, true);
            range = { from: 0, to: e.state.doc.content.size };
          }
        } else {
          if (onToastRef.current) onToastRef.current('warning', 'Неизвестный тип команды'); return false;
        }
        // Scroll to range start
        if (range) {
          setTimeout(() => {
            try {
              const dom = e.view.domAtPos(range.from);
        ...
        setPendingEdit({ cmd, range, original, onResolve: opts && opts.onResolve });
        return true;
      } catch (err) {
        console.error('previewAgentEdit failed', err);
        if (onToastRef.current) onToastRef.current('warning', 'Ошибка превью: ' + err.message);
        return false;
      }
    };
    window.cancelAgentEdit = () => { if (pendingEditRef.current) rejectPendingEdit(); };
    return () => { delete window.previewAgentEdit; delete window.cancelAgentEdit; };
  }, [rejectPendingEdit]);

  // Esc cancels, Ctrl+Enter accepts pending preview
  useEffect(() => {
    if (!pendingEdit) return;
    const onKey = (e) => {
      if (e.key === 'Escape') { e.preventDefault(); rejectPendingEdit(); }
      else if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') { e.preventDefault(); acceptPendingEdit(); }
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [pendingEdit, acceptPendingEdit, rejectPendingEdit]);

  // Recompute page count via ResizeObserver
  useEffect(() => {
    if (!ready || !elRef.current) return;
    const el = elRef.current;
    const recompute = () => {
      const contentPx = el.scrollHeight;
      const contentMm = contentPx * MM_PER_PX;
      const perPageMm = A4_HEIGHT_MM - PAD_TOP_MM - PAD_BOTTOM_MM;
      const pages = Math.max(1, Math.ceil(contentMm / perPageMm));
      setPageCount(prev => prev === pages ? prev : pages);
      if (editorRef.current && editorRef.current.storage.characterCount) {
    ...
    if (editor) editor.on('update', recompute);
    return () => { ro.disconnect(); if (editor) editor.off('update', recompute); };
  }, [ready]);
  ...
  useEffect(() => {
    ...
    const onWheel = (e) => {
      ...
      e.preventDefault();
      setZoom(z => {
        const idx = ZOOM_PRESETS.indexOf(z);
        if (idx === -1) {
          let nearest = ZOOM_PRESETS[0]; let best = Math.abs(z - nearest);
          for (const p of ZOOM_PRESETS) { const d = Math.abs(z - p); if (d < best) { best = d; nearest = p; } }
          return nearest;
        }
        const dir = e.deltaY < 0 ? 1 : -1;
        return ZOOM_PRESETS[Math.max(0, Math.min(ZOOM_PRESETS.length - 1, idx + dir))];
      });
    };
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      else if (e.key === '=' || e.key === '+') { e.preventDefault(); setZoom(z => { const i = ZOOM_PRESETS.indexOf(z); return ZOOM_PRESETS[Math.min(ZOOM_PRESETS.length - 1, i + 1)] || 1; }); }
      else if (e.key === '-') { e.preventDefault(); setZoom(z => { const i = ZOOM_PRESETS.indexOf(z); return ZOOM_PRESETS[Math.max(0, i - 1)] || 1; }); }
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, []);
  ...
  if (loadError) {
    return (
      <div style={{padding:24,margin:24,border:'1px solid var(--red)',background:'var(--red-soft)',borderRadius:10,color:'var(--text)',fontFamily:'monospace',fontSize:12}}>
        <div style={{fontWeight:700,marginBottom:8,color:'var(--red)'}}>⚠ Ошибка загрузки TipTap</div>
  ...
  return (
    <div className="docengine-shell" style={{flex:1, display:'flex', flexDirection:'column', minHeight:0}}>
      <TipTapToolbar editor={editorInst} />
      <TipTapRuler editor={editorInst} zoom={zoom} marginLeft={marginLeft} marginRight={marginRight} setMarginLeft={setMarginLeft} setMarginRight={setMarginRight} />
      <div className="docengine-canvas" ref={canvasRef}>
        <div className="docengine-page-wrapper">
          <div className="docengine-page">
            <div className="docengine-margin-guides" />
            <TipTapPageBreakOverlay pages={pageCount} />
            <div ref={elRef} className="docengine-editor-mount" />
          </div>
        </div>
      </div>
      {pendingEdit && pendingRect && (
        <div className="agent-floating-bar" style={{ left: pendingRect.left, top: pendingRect.top, transform: 'translateX(-50%)' }}>
          <span className="lbl">{pendingEdit.cmd.type==='replace_selection'?'Замена':pendingEdit.cmd.type==='replace_all'?'Полная замена':pendingEdit.cmd.type==='insert_end'?'Вставка в конец':pendingEdit.cmd.type==='insert_smart'?'Умная вставка':'Вставка'}</span>
          <button className="btn-accept" onClick={acceptPendingEdit}>
            <span>✓ Применить</span><span className="kbd">⌘↵</span>
          </button>
          <button className="btn-reject" onClick={rejectPendingEdit}>
            <span>✗ Отклонить</span><span className="kbd">Esc</span>
          </button>
        </div>
      )}
      <div className="docengine-statusbar">
        <div style={{display:'flex',alignItems:'center',gap:16}}>
          <span>Стр. {pageCount}</span>
          <span>Слов {wordCount.toLocaleString('ru-RU')}</span>
          <span>Русский</span>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          <div className="docengine-zoom" style={{display:'flex',alignItems:'center',gap:4}}>
            <button style={{border:'none',background:'transparent',cursor:'pointer',color:'var(--text-muted)'}} onClick={() => setZoom(z => ZOOM_PRESETS[Math.max(0, ZOOM_PRESETS.indexOf(z) - 1)] || 1)}>−</button>
            <span style={{minWidth:40,textAlign:'center'}}>{Math.round(zoom * 100)}%</span>
            <button style={{border:'none',background:'transparent',cursor:'pointer',color:'var(--text-muted)'}} onClick={() => setZoom(z => ZOOM_PRESETS[Math.min(ZOOM_PRESETS.length - 1, ZOOM_PRESETS.indexOf(z) + 1)] || 1)}>+</button>
          </div>
          <button style={{border:'none',background:'transparent',cursor:'pointer',color:'var(--text-muted)',display:'flex',alignItems:'center'}} title="На весь экран" onClick={()=>{if(!document.fullscreenElement) document.documentElement.requestFullscreen(); else document.exitFullscreen();}}><Ico k="maximize" sz={13}/></button>
        </div>
      </div>
    </div>
  );
...
const AIEditor = ({ onToast, onCtx }) => {
  ...
  const [quillInst, setQuillInst] = useState(null);
  const [pendingEdit, setPendingEdit] = useState(null);
  const [pendingRect, setPendingRect] = useState(null);
  const pendingEditRef = useRef(null);
  useEffect(()=>{pendingEditRef.current=pendingEdit},[pendingEdit]);
  ...
  const recomputePendingRect = useCallback(()=>{
    ...
    const pe=pendingEditRef.current;
    if(!q||!pe||!pe.range){setPendingRect(null);return}
    try{
      const bounds=q.getBounds(pe.range.index, Math.max(1,pe.range.length));
      const cb=q.container.getBoundingClientRect();
      const containerScroll=q.scrollingContainer?q.scrollingContainer.scrollTop:0;
      const top=cb.top+bounds.bottom-containerScroll;
      const left=cb.left+bounds.left+bounds.width/2;
      const visTop=Math.max(60,Math.min(window.innerHeight-90,top+10));
      const visLeft=Math.max(140,Math.min(window.innerWidth-140,left));
      setPendingRect({left:visLeft, top:visTop});
    }catch(e){setPendingRect(null)}
  },[]);
  ...
  useEffect(()=>{
    ...
    const q=quillRef.current;
    if(!q) return;
    const sc=q.scrollingContainer||q.root;
    const onScroll=()=>recomputePendingRect();
    const onResize=()=>recomputePendingRect();
    sc.addEventListener('scroll',onScroll,{passive:true});
    window.addEventListener('resize',onResize);
    return()=>{sc.removeEventListener('scroll',onScroll);window.removeEventListener('resize',onResize)};
  },[pendingEdit,recomputePendingRect]);
  ...
  const acceptPendingEdit=useCallback(()=>{
    ...
    if(!q||!pe) return;
    try{
      if(pe.cmd.type==='replace_selection' && pe.original){
        // delete the strike-through old text
        q.deleteText(pe.original.oldStart, pe.original.oldLen);
        // remove highlight from new text (which is now at oldStart)
        q.formatText(pe.original.oldStart, pe.cmd.text.length, 'agentPending', false);
      }else if(pe.cmd.type==='replace_all'){
        q.formatText(0, q.getLength()-1, 'agentPending', false);
      }else{
        q.formatText(pe.range.index, pe.range.length, 'agentPending', false);
      }
    }catch(e){console.error('accept failed',e)}
    setPendingEdit(null);
    onToast&&onToast('check','Применено');
    if(pe.onResolve) pe.onResolve('applied');
  },[onToast]);
  ...
  const rejectPendingEdit=useCallback(()=>{
    ...
    if(!q||!pe) return;
    try{
      if(pe.cmd.type==='insert_end'||pe.cmd.type==='insert_cursor'||pe.cmd.type==='insert_smart'){
        q.deleteText(pe.range.index, pe.range.length);
      }else if(pe.cmd.type==='replace_selection' && pe.original){
        // delete the new (highlighted) text
        q.deleteText(pe.original.oldStart+pe.original.oldLen, pe.cmd.text.length);
        // remove deletion format from original
        q.formatText(pe.original.oldStart, pe.original.oldLen, 'agentPending', false);
      }else if(pe.cmd.type==='replace_all' && pe.original){
        q.setContents(pe.original.delta);
      }
    }catch(e){console.error('reject failed',e)}
    setPendingEdit(null);
    onToast&&onToast('x','Отклонено');
    if(pe.onResolve) pe.onResolve('rejected');
  },[onToast]);
  ...
  useEffect(()=>{
    window.previewAgentEdit = (cmd, opts)=>{
      const q=quillRef.current;
      if(!q){onToast&&onToast('warning','Редактор не готов');return false}
      // Cancel any existing preview first
      if(pendingEditRef.current){
        rejectPendingEdit();
      }
      const text=String(cmd.text||'').trim();
      if(!text){onToast&&onToast('warning','Пустой текст');return false}
      let range=null;
      let original=null;
      try{
        // Helper: merge format from preceding char into the agentPending mark
        // so вставленный текст получит тот же шрифт/размер/выравнивание окружения.
        const buildFormats=(pos)=>{
          try{
            const probe=Math.max(0, Math.min(pos-1, q.getLength()-1));
            const f=q.getFormat(probe)||{};
            // Не переносим diff-marker от предыдущей правки
            delete f.agentPending;
            return {...f, agentPending:'insert'};
          }catch(_){return {agentPending:'insert'};}
        };
        if(cmd.type==='insert_smart'){
          // Quill: plain-text indexOf works directly because Quill's text is flat.
          const anchor=String(cmd.anchor||'').trim();
          let insertPos=-1;
          if(anchor && anchor.length>=3 && anchor!=='EMPTY'){
            const fullText=q.getText();
            const idx=fullText.toLowerCase().indexOf(anchor.toLowerCase());
            if(idx!==-1) insertPos=idx+anchor.length;
          }
          if(insertPos===-1){
            // Fallback: anchor not found OR EMPTY → append to end
            if(anchor && anchor!=='EMPTY'){
              console.warn('[Agent] Anchor not found in Quill, falling back to insert_end. Anchor:', anchor);
              onToast&&onToast('warning','Якорь не найден — вставлено в конец');
            }
            insertPos=Math.max(0,q.getLength()-1);
          }
          const ins='\n'+text+'\n';
          q.insertText(insertPos, ins, buildFormats(insertPos));
          range={index:insertPos, length:ins.length};
        }else if(cmd.type==='insert_end'){
          const idx=Math.max(0,q.getLength()-1);
          const ins='\n'+text+'\n';
          q.insertText(idx, ins, buildFormats(idx));
          range={index:idx, length:ins.length};
        }else if(cmd.type==='insert_cursor'){
          const sel=q.getSelection();
          const idx=sel?sel.index:Math.max(0,q.getLength()-1);
          q.insertText(idx, text, buildFormats(idx));
          range={index:idx, length:text.length};
        }else if(cmd.type==='replace_selection'){
          const sel=q.getSelection();
          if(!sel||sel.length===0){onToast&&onToast('warning','Нет выделения для замены');return false}
          const oldStart=sel.index, oldLen=sel.length;
          q.formatText(oldStart, oldLen, 'agentPending', 'delete');
          // Наследуем форматирование заменяемого текста
          q.insertText(oldStart+oldLen, text, buildFormats(oldStart));
          range={index:oldStart, length:oldLen+text.length};
          original={oldStart, oldLen};
        }else if(cmd.type==='replace_all'){
          const delta=q.getContents();
          q.setText(text+'\n');
          q.formatText(0, text.length, 'agentPending', 'insert');
          range={index:0, length:text.length};
          original={delta};
        }else{
          onToast&&onToast('warning','Неизвестный тип команды');return false
        }
        // Scroll to the change
        if(range){
          setTimeout(()=>{
            try{
              const bounds=q.getBounds(range.index, Math.min(range.length,1));
              const sc=q.scrollingContainer||q.root;
              if(sc&&typeof sc.scrollTo==='function'){
                sc.scrollTo({top:Math.max(0,bounds.top-120),behavior:'smooth'});
              }
            }catch(e){}
          },50);
        }
        ...
        console.error('previewAgentEdit failed',e);
        onToast&&onToast('warning','Ошибка превью: '+e.message);
        return false;
      }
    };
    window.cancelAgentEdit = ()=>{ if(pendingEditRef.current) rejectPendingEdit(); };
    return ()=>{ delete window.previewAgentEdit; delete window.cancelAgentEdit; };
  },[onToast,rejectPendingEdit]);

  // Esc key cancels pending edit
  useEffect(()=>{
    if(!pendingEdit) return;
    const onKey=(e)=>{
      if(e.key==='Escape'){e.preventDefault();rejectPendingEdit()}
  ...
  useEffect(() => {
    if (editorRef.current && !quillRef.current && window.Quill) {
      try {
        ...
        Font.whitelist = ['sans-serif', 'serif', 'monospace', 'times-new-roman', 'arial', 'georgia', 'verdana', 'courier-new'];
        window.Quill.register(Font, true);

        const Size = window.Quill.import('attributors/style/size');
        Size.whitelist = ['10px','11px','12px','13px','14px','16px','18px','20px','22px','24px','28px','32px','36px','48px'];
        window.Quill.register(Size, true);

        const Parchment = window.Quill.import('parchment');
        if (!window.__customQuillFormatsRegistered && Parchment && Parchment.Attributor && Parchment.Attributor.Style) {
          window.__customQuillFormatsRegistered = true;
          
          const IndentFirstLineStyle = new Parchment.Attributor.Style('indentFirstLine', 'text-indent', {
            scope: Parchment.Scope.BLOCK
          });
          window.Quill.register(IndentFirstLineStyle, true);

          const MarginLeftStyle = new Parchment.Attributor.Style('marginLeftBlock', 'margin-left', {
            scope: Parchment.Scope.BLOCK
          });
          window.Quill.register(MarginLeftStyle, true);

          const MarginRightStyle = new Parchment.Attributor.Style('marginRightBlock', 'margin-right', {
            scope: Parchment.Scope.BLOCK
          });
          window.Quill.register(MarginRightStyle, true);

          const AgentPending = new Parchment.Attributor.Class('agentPending', 'agent-pending', {
            scope: Parchment.Scope.INLINE,
            whitelist: ['insert', 'delete']
          });
          window.Quill.register(AgentPending, true);
        }
        ...
        quillRef.current = new window.Quill(editorRef.current, {
          ...
          modules: {
            toolbar: [
              [{ 'font': ['times-new-roman', '', 'sans-serif', 'serif', 'arial', 'georgia', 'verdana', 'courier-new', 'monospace'] }],
        ...
        window.currentQuill = quillRef.current;
        setQuillInst(quillRef.current);

        // Set default font and size
        quillRef.current.format('font', 'times-new-roman');
        ...
        quillRef.current.on('selection-change', (range) => {
          if (range && range.length > 5) {
            ...
            const containerBounds = quillRef.current.container.getBoundingClientRect();
            setFloatRect({
              left: containerBounds.left + bounds.left,
              top: containerBounds.top + bounds.top,
              bottom: containerBounds.top + bounds.top + bounds.height,
              width: bounds.width
            });
            setSelRange(range);
          } else {
        ...
        if(onToast) onToast('warning', 'Quill Init Fail: ' + e.message);
        console.error('Quill crashed:', e);
      }

      quillRef.current.root.addEventListener('contextmenu', (e) => {
        e.preventDefault();
        onCtx(e.clientX, e.clientY, [{label:'Улучшить текст', icon:'sparkles', action: () => document.getElementById('improve-btn')?.click()}]);
      });
    }
  }, [onToast]);

  const handleImprove = async () => {
    if (!selRange || !quillRef.current) return;
    const textToImprove = quillRef.current.getText(selRange.index, selRange.length);
    setIsImproving(true);
    try {
      const res = await fetch(BACKEND_URL+'/api/edit', {
        ...
        body: JSON.stringify({
          text: textToImprove,
          instruction: 'Перепиши этот текст строгим канцелярским юридическим языком, исправь ошибки'
        })
      });
      const data = await res.json();
      if (data.result) {
        quillRef.current.deleteText(selRange.index, selRange.length);
        quillRef.current.insertText(selRange.index, data.result);
        quillRef.current.setSelection(selRange.index, data.result.length);
        onToast('sparkles', 'Текст улучшен');
      } else {
        onToast('warning', 'Ошибка редактирования');
      }
    } catch (e) {
      onToast('warning', 'Ошибка сети');
    } finally {
  ...
  return (
    <div style={{flex:1, display:'flex', flexDirection:'column', position:'relative', minWidth:0, minHeight:0, background:'var(--bg-editor)'}}>
      <ErrorBoundary>
        <DocumentRuler quill={quillInst} />
      </ErrorBoundary>
      <div ref={editorRef} className="myz-editor" style={{flex:1, minHeight:0, display:'flex', flexDirection:'column'}} />
      {floatRect && !pendingEdit && (
        <div style={{position:'fixed',zIndex:100,left:floatRect.left+floatRect.width/2,top:floatRect.bottom+16,transform:'translateX(-50%)', display:'flex', gap: 4}} className="flt-bar">
          <div style={{display:'flex',alignItems:'center',gap:4,background:'var(--bg-panel)',borderRadius:20,border:'1px solid var(--accent)',padding:'4px 6px',boxShadow:'var(--shadow-lg)'}}>
            <button id="improve-btn" className="btn" onClick={handleImprove} disabled={isImproving} style={{display:'flex',alignItems:'center',gap:6,padding:'4px 10px',background:'transparent',border:'none',borderRadius:16,color:'var(--text)',fontSize:12,fontWeight:600,cursor:isImproving?'wait':'pointer'}}>
              <Ico k={isImproving ? 'loader' : 'sparkles'} sz={14} col="var(--accent)" grad glow style={isImproving ? {animation:'spin 1s linear infinite'} : {}}/>
              {isImproving ? <span>Улучшаем...</span> : <span>✨ Улучшить текст</span>}
            </button>
          </div>
        </div>
      )}
      {pendingEdit && pendingRect && (
        <div className="agent-floating-bar" style={{left:pendingRect.left, top:pendingRect.top, transform:'translateX(-50%)'}}>
          <Ico k="sparkles" sz={13} col="var(--accent)" grad glow/>
          <span className="lbl">{pendingEdit.cmd.type==='replace_selection'?'Замена':pendingEdit.cmd.type==='replace_all'?'Полная замена':pendingEdit.cmd.type==='insert_end'?'Вставка в конец':pendingEdit.cmd.type==='insert_smart'?'Умная вставка':'Вставка'}</span>
          <button className="btn-accept" onClick={acceptPendingEdit}>
            <Ico k="check" sz={12} col="#fff"/>
            <span>Применить</span>
...
/* ═══ Editor ═══ */
const EditorComp=({tabs,active,onTab,onClose,onRef,hilite,onReorderTabs,onCtx,onToast,onAction,showFind,setShowFind,splitActive})=>{
  const dragIdx=useRef(null);const[dragOver,setDragOver]=useState(null);
  const handleDrop=useCallback(e=>{e.preventDefault();const n=e.dataTransfer.getData('text/plain');if(n)onAction('openFile',n)},[onAction]);
  const handleTabCtx=(e,tab)=>{e.preventDefault();e.stopPropagation();onCtx(e.clientX,e.clientY,[{label:'Закрыть',shortcut:'Ctrl+W',action:()=>onClose(tab.id)},{label:'Закрыть другие',action:()=>tabs.filter(t=>t.id!==tab.id).forEach(t=>onClose(t.id))},{label:'Закрыть все',action:()=>onAction('closeAllTabs')},{sep:true},{label:'Копировать путь',action:()=>{navigator.clipboard&&navigator.clipboard.writeText(tab.name);onToast('copy','Скопировано')}}])};

  if(!tabs.length) return(
    <div style={{height:'100%',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--bg-editor)'}} onDrop={handleDrop} onDragOver={e=>e.preventDefault()}>
      <EmptyIllust/>
    </div>
  );
  ...
  const editorPane=(
    <div style={{height:'100%',display:'flex',flexDirection:'column',background:'var(--bg-editor)'}}>
      <div className="myz-tab-bar" style={{height:38,flexShrink:0,background:'var(--bg-panel)',borderBottom:'1px solid var(--border-color)',display:'flex',alignItems:'stretch',overflow:'hidden',transition:'background-color .3s, border-color .3s',fontFamily:'var(--font-sans)'}}>
        {tabs.map((t,i)=>{
          const on=t.id===active;const isDO=dragOver===i&&dragIdx.current!==i;
          return(
            <div key={t.id} className="myz-tab" data-active={on?'true':'false'} draggable onDragStart={()=>{dragIdx.current=i}} onDragOver={e=>{e.preventDefault();setDragOver(i)}} onDragLeave={()=>setDragOver(null)} onDrop={e=>{e.preventDefault();if(dragIdx.current!==null&&dragIdx.current!==i)onReorderTabs(dragIdx.current,i);dragIdx.current=null;setDragOver(null)}} onClick={()=>onTab(t.id)} onContextMenu={e=>handleTabCtx(e,t)} style={{display:'flex',alignItems:'center',gap:'var(--s-2)',padding:'0 var(--s-3h)',cursor:'grab',borderRight:'1px solid var(--border-color)',flexShrink:0,userSelect:'none',background:on?'var(--bg-input)':isDO?'var(--hover)':'transparent',color:on?'var(--text-main)':'var(--text-muted)',position:'relative',fontSize:'var(--text-sm)',fontWeight:on?600:500,letterSpacing:'-.005em',opacity:dragIdx.current===i?.5:1,transition:'background-color .3s, color .3s, border-color .3s'}}>
              {/* Active accent strip — bottom border, refined */}
              {on && <span style={{position:'absolute',left:0,right:0,bottom:0,height:2,background:'var(--primary)'}}/>}
              {/* Dirty indicator dot — left of name */}
              {t.mod && <span title="Не сохранено" style={{width:7,height:7,borderRadius:'50%',background:'var(--primary)',flexShrink:0}}/>}
              {!t.mod && <Ico k="file" sz={14} col={on?'var(--primary)':'var(--text-muted)'} {...(on?{duo:true,fill:'var(--primary)'}:{})}/>}
              <span style={{maxWidth:160,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{t.name}</span>
              <button type="button" className="myz-tab-close" onClick={e=>{e.stopPropagation();onClose(t.id)}} title="Закрыть" aria-label={'Закрыть '+t.name} style={{display:'flex',alignItems:'center',justifyContent:'center',borderRadius:'var(--radius-xs)',border:'none',background:'transparent',padding:'var(--s-half)',cursor:'pointer',color:'var(--text-muted)',transition:'background .12s, color .12s',marginLeft:'var(--s-half)',position:'relative'}} onMouseEnter={e=>{e.currentTarget.style.background='var(--hover)';e.currentTarget.style.color='var(--text-main)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--text-muted)'}}><Ico k="x" sz={12}/><span aria-hidden="true" style={{position:'absolute',inset:-6}}/></button>
            </div>
          );
        })}
        <div style={{flex:1}}/>
      </div>
      <div style={{height:USE_TIPTAP?28:34,flexShrink:0,background:'var(--bg-panel)',borderBottom:'1px solid var(--border-color)',display:'flex',alignItems:'center',padding:USE_TIPTAP?'0 var(--s-2)':'0 var(--s-2h)',gap:1,transition:'background-color .3s, border-color .3s',fontFamily:'var(--font-sans)'}}>
        {!USE_TIPTAP && <>
        {[['undo',14],['redo',14]].map(([k,s])=>(<button key={k} onClick={()=>{if(window.currentQuill){const h=window.currentQuill.history; k==='undo'?h.undo():h.redo();}}} className="btn" style={{width:26,height:26,border:'none',borderRadius:'var(--radius-sm)',background:'transparent',cursor:'pointer',color:'var(--text-muted)',display:'flex',alignItems:'center',justifyContent:'center'}} onMouseEnter={e=>{e.currentTarget.style.background='var(--hover)';e.currentTarget.style.color='var(--text-main)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--text-muted)'}}><Ico k={k} sz={s}/></button>))}
        <div style={{width:1,height:16,background:'var(--border)',margin:'0 var(--s-1)'}}/>
        {[['bold',14],['italic',14],['underl',14],['strike',14]].map(([k,s])=>(<button key={k} onClick={()=>{if(window.currentQuill){const fmt=k==='underl'?'underline':k==='strike'?'strike':k;const f=window.currentQuill.getFormat(); window.currentQuill.format(fmt, !f[fmt]);}}} className="btn" style={{width:26,height:26,border:'none',borderRadius:'var(--radius-sm)',background:'transparent',cursor:'pointer',color:'var(--text)',display:'flex',alignItems:'center',justifyContent:'center'}} onMouseEnter={e=>e.currentTarget.style.background='var(--hover)'} onMouseLeave={e=>e.currentTarget.style.background='transparent'}><Ico k={k} sz={s}/></button>))}
        <div style={{width:1,height:16,background:'var(--border)',margin:'0 var(--s-1)'}}/>
        {[['align-left',14,''],['align-center',14,'center'],['align-right',14,'right'],['align-justify',14,'justify']].map(([k,s,v])=>(<button key={k} onClick={()=>{if(window.currentQuill) window.currentQuill.format('align',v||false);}} className="btn" style={{width:26,height:26,border:'none',borderRadius:'var(--radius-sm)',background:'transparent',cursor:'pointer',color:'var(--muted)',display:'flex',alignItems:'center',justifyContent:'center'}} onMouseEnter={e=>{e.currentTarget.style.background='var(--hover)';e.currentTarget.style.color='var(--text)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--muted)'}}><Ico k={k} sz={s}/></button>))}
        <div style={{width:1,height:16,background:'var(--border)',margin:'0 var(--s-1)'}}/>
        {[['list',14,'ordered'],['list',14,'bullet']].map(([k,s,v],idx)=>(<button key={v} onClick={()=>{if(window.currentQuill){const f=window.currentQuill.getFormat(); window.currentQuill.format('list',f.list===v?false:v);}}} className="btn" style={{width:26,height:26,border:'none',borderRadius:'var(--radius-sm)',background:'transparent',cursor:'pointer',color:'var(--muted)',display:'flex',alignItems:'center',justifyContent:'center'}} onMouseEnter={e=>{e.currentTarget.style.background='var(--hover)';e.currentTarget.style.color='var(--text)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--muted)'}}><Ico k={v==='ordered'?'list-ordered':'list'} sz={s}/></button>))}
        <div style={{width:1,height:16,background:'var(--border)',margin:'0 var(--s-1)'}}/>
        <button onClick={()=>{if(window.currentQuill){const f=window.currentQuill.getFormat(); window.currentQuill.format('indent','+1');}}} className="btn" style={{width:26,height:26,border:'none',borderRadius:'var(--radius-sm)',background:'transparent',cursor:'pointer',color:'var(--muted)',display:'flex',alignItems:'center',justifyContent:'center'}} onMouseEnter={e=>{e.currentTarget.style.background='var(--hover)';e.currentTarget.style.color='var(--text)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--muted)'}} title="Увеличить отступ"><Ico k="indent" sz={14}/></button>
        <button onClick={()=>{if(window.currentQuill){const f=window.currentQuill.getFormat(); window.currentQuill.format('indent','-1');}}} className="btn" style={{width:26,height:26,border:'none',borderRadius:'var(--radius-sm)',background:'transparent',cursor:'pointer',color:'var(--muted)',display:'flex',alignItems:'center',justifyContent:'center'}} onMouseEnter={e=>{e.currentTarget.style.background='var(--hover)';e.currentTarget.style.color='var(--text)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--muted)'}} title="Уменьшить отступ"><Ico k="outdent" sz={14}/></button>
        </>}
        <div style={{flex:1}}/>
        <button onClick={()=>onAction('save')} className="btn" title="Сохранить (Ctrl+S)" style={{display:'flex',alignItems:'center',gap:'var(--s-1)',padding:'var(--s-half) var(--s-2)',height:22,border:'1px solid var(--border)',borderRadius:'var(--radius-sm)',background:'transparent',cursor:'pointer',color:'var(--muted)',fontSize:'var(--text-xs)',fontFamily:'inherit'}} onMouseEnter={e=>{e.currentTarget.style.background='var(--hover)';e.currentTarget.style.color='var(--text)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--muted)'}}><Ico k="save" sz={12}/><span>Сохранить</span></button>
        {tabs.find(t=>t.id===active)?.buffer && <button onClick={()=>onAction('showOriginal')} className="btn" title="Показать оригинал DOCX" style={{display:'flex',alignItems:'center',gap:'var(--s-1)',padding:'var(--s-half) var(--s-2)',height:22,marginLeft:'var(--s-1)',border:'1px solid var(--border)',borderRadius:'var(--radius-sm)',background:'transparent',cursor:'pointer',color:'var(--muted)',fontSize:'var(--text-xs)',fontFamily:'inherit'}} onMouseEnter={e=>{e.currentTarget.style.background='var(--hover)';e.currentTarget.style.color='var(--text)'}} onMouseLeave={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--muted)'}}><Ico k="book" sz={12}/><span>Оригинал</span></button>}
        <button onClick={()=>{window.print();onToast('pdf','Печать / PDF');}} className="btn" title="Печать / PDF (Ctrl+P)" style={{display:'flex',alignItems:'center',gap:'var(--s-1)',padding:'var(--s-half) var(--s-2)',height:22,border:'none',marginLeft:'var(--s-1)',borderRadius:'var(--radius-sm)',background:'linear-gradient(135deg,var(--accent),var(--accent2))',cursor:'pointer',color:'#fff',fontSize:'var(--text-xs)',fontFamily:'inherit',fontWeight:500,boxShadow:'0 1px 6px var(--accent-glow)'}} onMouseEnter={e=>e.currentTarget.style.opacity='.85'} onMouseLeave={e=>e.currentTarget.style.opacity='1'}><Ico k="pdf" sz={12} col="#fff"/><span>PDF</span></button>
      </div>
      {showFind && <div style={{height:26,flexShrink:0,background:'var(--bg-editor)',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',padding:'0 12px',position:'relative'}}>
        <FindBar onClose={()=>setShowFind(false)} onToast={onToast}/>
      </div>}
  ...
  if(splitActive) return(<div style={{height:'100%',display:'flex',overflow:'hidden'}}><div style={{flex:1,overflow:'hidden',borderRight:'1px solid var(--border)'}}>{editorPane}</div><div style={{width:4,flexShrink:0,background:'var(--border)',cursor:'col-resize'}}/><div style={{flex:1,overflow:'hidden',opacity:.4}}>{editorPane}</div></div>);
  return editorPane;
};
...
const AIChat=({onToast,onOpenArticle,onCollapse})=>{
  ...
  useEffect(() => {
    ...
    const tryFireShadow = () => {
      ...
      if (!e || typeof e.getText !== 'function') return;
      const text = String(e.getText() || '');
    ...
    const tryAttach = () => {
      const e = window.docEngine;
      if (!e || typeof e.on !== 'function') return false;
      e.on('update', onEditorUpdate);
      detach = () => { try { e.off('update', onEditorUpdate); } catch (err) {} };
      // Один прогон сразу — на случай если документ уже загружен
      // (например юрист вставил текст до того как мы подписались).
      onEditorUpdate();
      return true;
    };

    if (!tryAttach()) {
      // docEngine ещё не готов — поллим раз в 500мс, пока не появится
      waitInterval = setInterval(() => {
        if (tryAttach()) { clearInterval(waitInterval); waitInterval = null; }
      }, 500);
    }
    return () => {
      if (detach) detach();
  ...
  const send=async()=>{
    ...
    if(agent){
      ...
      console.log('[Agent] Document snapshot:',{
        editorReady:editorAdapter.isReady(),
  ...
  useEffect(() => {
    if (!editorAdapter.isReady() || typeof window.previewAgentEdit !== 'function') return;
      ...
      <div style={{padding:'var(--s-2) var(--s-2h)',borderTop:'1px solid var(--border)',flexShrink:0,background:'var(--bg-bar)',display: chatMode === 'analyze-docs' ? 'none' : 'block'}}>
        ...
        <div style={{position:'relative',display:'flex',alignItems:'flex-end',gap:'var(--s-1h)',border:'1px solid var(--border-color)',borderRadius:'var(--radius)',background:'var(--bg-input)',transition:'border-color .3s, box-shadow .3s',padding:'var(--s-2h) var(--s-3)'}} onFocusCapture={e=>{e.currentTarget.style.borderColor='var(--primary)';e.currentTarget.style.boxShadow='0 0 0 3px var(--accent-dim)'}} onBlurCapture={e=>{e.currentTarget.style.borderColor='var(--border-color)';e.currentTarget.style.boxShadow='none'}}>
          ...
          <textarea
            ...
            placeholder={agent ? 'Спросить о документе…' : 'Задать юридический вопрос…'}
...
const StatusBar=({dark,tabCount,unsaved,activeName})=>{
  ...
  useEffect(()=>{
    let detach=null,interval=null;
    const update=()=>{
      const e=window.docEngine;
      if(!e||!e.storage?.characterCount) return;
      setStats({words:e.storage.characterCount.words()||0,chars:e.storage.characterCount.characters()||0});
    };
    const tryAttach=()=>{
      const e=window.docEngine;
      if(!e) return false;
      ...
      detach=()=>{ try{e.off('update',update);e.off('selectionUpdate',update);}catch(err){} };
      return true;
    };
    if(!tryAttach()){
      interval=setInterval(()=>{ if(tryAttach()){clearInterval(interval);interval=null;} },500);
    }
    return ()=>{ if(detach)detach(); if(interval)clearInterval(interval); };
  },[]);
  return (
    <div style={{height:24,flexShrink:0,background:dark?'var(--bg-panel)':'var(--primary)',borderTop:dark?'1px solid var(--border-color)':'none',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 var(--s-3)',fontSize:'var(--text-xs)',userSelect:'none',color:dark?'var(--text-muted)':'#ffffff',transition:'background-color .3s, color .3s, border-color .3s',fontFamily:'var(--font-sans)'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:'var(--s-3h)'}}>
        <span style={{opacity:.65,fontFamily:'var(--font-mono)'}}>{stats.words.toLocaleString('ru-RU')} слов · {stats.chars.toLocaleString('ru-RU')} симв.</span>
        <span style={{opacity:.5,fontFamily:'var(--font-mono)'}}>ru</span>
...
/* ═══ Magic Wand ═══ */
const MagicWandTooltip = ({ onToast }) => {
  ...
  useEffect(() => {
    const handleSelection = () => {
      setTimeout(() => {
        if(!editorAdapter.isReady()){ setPos(null); return; }
        const info = editorAdapter.getSelectionInfo();
        if (info && info.hasSelection && info.text.trim().length > 0) {
  ...
  const handleMagic = async (type) => {
    ...
    if (type === 'rewrite') prompt = 'Перепиши этот текст, сделав его более профессиональным и юридически точным. Выдай ТОЛЬКО текст, без кавычек и комментариев:\n\n' + selText;
    else if (type === 'simplify') prompt = 'Сделай этот текст проще для понимания обывателю. Выдай ТОЛЬКО текст, без кавычек и комментариев:\n\n' + selText;
    else if (type === 'translate') prompt = 'Переведи этот текст на английский язык. Выдай ТОЛЬКО текст, без кавычек и комментариев:\n\n' + selText;
    ...
    try {
      ...
      if (finalRes) {
        if (USE_TIPTAP && window.docEngine) {
          window.docEngine.chain().focus().deleteRange({from: selInfo.from, to: selInfo.to}).insertText(finalRes).run();
        } else if (window.currentQuill) {
          const q = window.currentQuill;
          const format = q.getFormat(selInfo.index, 1);
          q.deleteText(selInfo.index, selInfo.length);
          q.insertText(selInfo.index, finalRes, format);
        }
        onToast('sparkles', 'Текст изменён');
      }
    } catch (e) {
      onToast('warning', 'Ошибка ИИ');
    }
    setLoading(false);
    setPos(null);
  };
  ...
  return (
    <div style={{
      ...
      boxShadow: 'var(--shadow-lg)', padding: '4px', zIndex: 10000, display: 'flex', gap: '4px',
      alignItems: 'center', animation: 'fadeInScale 0.15s ease'
    }} onMouseDown={preventFocusLoss}>
      <div style={{color: 'var(--accent)', padding: '0 6px', display: 'flex', alignItems: 'center'}}>
        <Ico k="sparkles" sz={14} grad glow />
      </div>
      <button disabled={loading} onClick={()=>handleMagic('rewrite')} className="btn" style={{padding: '4px 8px', fontSize: 12, borderRadius: '4px', cursor: loading?'wait':'pointer', background: 'transparent', color: 'var(--text)', border: 'none', fontWeight: 500}}>Улучшить</button>
      <button disabled={loading} onClick={()=>handleMagic('simplify')} className="btn" style={{padding: '4px 8px', fontSize: 12, borderRadius: '4px', cursor: loading?'wait':'pointer', background: 'transparent', color: 'var(--text)', border: 'none', fontWeight: 500}}>Упростить</button>
      <button disabled={loading} onClick={()=>handleMagic('translate')} className="btn" style={{padding: '4px 8px', fontSize: 12, borderRadius: '4px', cursor: loading?'wait':'pointer', background: 'transparent', color: 'var(--text)', border: 'none', fontWeight: 500}}>En</button>
    </div>
  );
};
...
const App=()=>{
  ...
  const [inlinePrompt, setInlinePrompt] = useState({ visible: false, top: 0, left: 0, pos: null, isStreaming: false });
  ...
  const switchTab=useCallback((newId)=>{
    ...
    if(editorAdapter.isReady() && activeTab){
      const content=editorAdapter.getHTML();
      setTabs(p=>p.map(t=>t.id===activeTab?{...t,content}:t));
    }
    setActiveTab(newId);
    setTabs(p=>{
      const next=p.find(t=>t.id===newId);
      setTimeout(()=>{
        if(editorAdapter.isReady() && next) editorAdapter.setHTML(next.content||'<p></p>');
      },10);
      return p;
    });
  },[activeTab]);
  ...
  const closeTab=useCallback(id=>{
    ...
    setActiveTab(prev=>{
      if(prev===id){
        ...
        if(nextId){
           setTimeout(()=>{if(editorAdapter.isReady()) editorAdapter.setHTML(r[0].content||'<p></p>')},10);
        } else {
           setTimeout(()=>{if(editorAdapter.isReady()) editorAdapter.setHTML('<p></p>')},10);
        }
        return nextId;
      }
      return prev;
  ...
  useEffect(()=>{const h=(e)=>{if(inlinePrompt.visible&&!e.target.closest('#inline-prompt-menu')){setInlinePrompt(p=>({...p,visible:false}))}};document.addEventListener('mousedown',h);return()=>document.removeEventListener('mousedown',h)},[inlinePrompt.visible]);
  ...
  return(
    <div className={(dark?'dk ':'')+(tt?'tt ':'')+'grain'} style={{width:'100vw',height:'100vh',display:'flex',flexDirection:'column',background:'var(--bg-editor)',color:'var(--text)',overflow:'hidden',fontFamily:"'DM Sans',ui-sans-serif,system-ui,sans-serif",fontSize:13.5,letterSpacing:'-.01em'}}>
      ...
      <MagicWandTooltip onToast={addToast} />
      {inlinePrompt.visible && (
        <div id="inline-prompt-menu" style={{
          position: 'fixed', zIndex: 9999,
          top: Math.max(0, inlinePrompt.top - 40), left: Math.max(0, inlinePrompt.left - 10),
          ...
          <input 
            type="text" autoFocus disabled={inlinePrompt.isStreaming}
            placeholder={
              inlinePrompt.isStreaming ? "Генерация текста..." : 
              inlinePrompt.streamStartPos !== null ? "Нажмите Tab чтобы принять, Esc чтобы отменить" : 
              "Спросите ИИ (изменить или дополнить текст)..."
            }
            style={{
              flex: 1, border: 'none', outline: 'none', background: 'transparent',
              fontSize: '13.5px', color: 'var(--text)', fontFamily: 'inherit',
              opacity: inlinePrompt.isStreaming ? 0.6 : 1,
              transition: 'all 0.3s ease'
            }}
            onKeyDown={e => {
              if (e.key === 'Escape') {
                e.preventDefault();
                if (inlinePrompt.isStreaming && window.__inlineAbort) {
                  window.__inlineAbort.abort();
                }
                try {
                  if (inlinePrompt.streamStartPos !== null && inlinePrompt.streamEndPos !== null && window.docEngine) {
                    const view = window.docEngine.view;
                    const tr = view.state.tr;
                    if (inlinePrompt.streamEndPos <= view.state.doc.content.size) {
                      tr.delete(inlinePrompt.streamStartPos, inlinePrompt.streamEndPos);
                      view.dispatch(tr);
                    }
                  }
                } catch(err) { console.error('Delete error', err); }
                setInlinePrompt(p => ({ ...p, visible: false, isStreaming: false, streamStartPos: null, streamEndPos: null }));
                ...
                if (inlinePrompt.streamStartPos !== null && inlinePrompt.streamEndPos !== null && window.docEngine) {
                  ...
                  try {
                    ...
                    if (schema.marks.agentPending && inlinePrompt.streamEndPos <= view.state.doc.content.size) {
                      tr.removeMark(inlinePrompt.streamStartPos, inlinePrompt.streamEndPos, schema.marks.agentPending);
                      view.dispatch(tr);
                    }
              ...
              } else if (e.key === 'Enter' && !inlinePrompt.isStreaming && inlinePrompt.streamStartPos === null) {
                e.preventDefault();
                const val = e.target.value.trim();
                if (val) {
                  const startPos = inlinePrompt.pos;
                  setInlinePrompt(p => ({ ...p, isStreaming: true, streamStartPos: startPos }));
                  ...
                  let context = '';
                  if (window.docEngine) {
                    const txt = window.docEngine.getText();
                  ...
                  streamAIResponse(val, context, (textToken) => {
                    ...
                    setInlinePrompt(p => ({ ...p, isStreaming: false, streamEndPos: currentPos }));
                  }).catch(() => {
                    setInlinePrompt(p => ({ ...p, isStreaming: false, streamEndPos: currentPos }));
                  });
                }
          ...
          />
          {inlinePrompt.streamStartPos !== null && !inlinePrompt.isStreaming && (
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden',position:'relative'}}>
        ...
        <div style={isMobile
          ...
          : {width:leftOpen?leftW:0,flexShrink:0,overflow:'hidden',borderRight:leftOpen?'1px solid var(--border)':'none',transition:'width .2s ease-in-out'}}>
        ...
        <div style={{flex:1,overflow:'hidden',minWidth:isMobile?0:200}}><EditorComp tabs={tabs} active={activeTab} onTab={switchTab} onClose={closeTab} onRef={handleRef} hilite={hilite} onReorderTabs={(f,t)=>{setTabs(p=>{const n=[...p];const[m]=n.splice(f,1);n.splice(t,0,m);return n})}} onCtx={(x,y,items)=>setCtxMenu({x,y,items})} onToast={addToast} onAction={handleAction} showFind={showFind} setShowFind={setShowFind} splitActive={splitActive}/></div>
        ...
        <div style={isMobile
          ...
          : {width:rightOpen?rightW:0,flexShrink:0,overflow:'hidden',background:'var(--bg-app)',borderLeft:rightOpen?'1px solid var(--border)':'none',transition:'width .2s ease-in-out'}}>
          {rightOpen && (
