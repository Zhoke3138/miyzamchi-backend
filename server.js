...
- ИЗМЕНИТЬ существующее (сумма, дата, ФИО, формулировка) → "op":"replace". old_text — это ТОЧНАЯ существующая фраза из документа (скопируй буквально, посимвольно, с теми же знаками/регистром), new_text — на что заменить. НЕ дублируй текст — именно замена.
- ДОБАВИТЬ новый пункт/абзац → "op":"insert_after" с anchor (фраза, ПОСЛЕ которой вставить) ЛИБО "op":"insert_end" (в конец документа).
- old_text и anchor КОПИРУЙ ДОСЛОВНО из ТЕКУЩЕГО документа — иначе фрагмент не найдётся поиском по тексту.
- Можно вернуть НЕСКОЛЬКО команд (например, заменить и сумму, и дату).
- При АНАЛИЗЕ (без правки) → "commands": [] (пустой массив), весь ответ в reasoning.
