...
const systemInstruction = [
    ...
    "Твоя задача — помогать гражданам разобраться в законодательстве КР: объяснить права, рассказать о порядке действий, разобраться в ситуации.",
    "Говоришь простым понятным языком — без лишнего юридического жаргона, но точно по закону.",
    "",
    "---",
    "",
    ...
    "- Запрос на документ → объясни что должно быть в документе + рекомендуй к юристу",
    ...
    "5. **Если контекст НПА пуст или не содержит ответа на ЮРИДИЧЕСКИЙ вопрос:** Скажи «В моей базе нет данных по этому вопросу. Рекомендую сверить с cbd.minjust.gov.kg или обратиться к юристу.» ЗАПРЕЩЕНО отвечать по праву КР из памяти — только из переданного контекста.",
    "6. **ЗАПРЕТ НА ГАЛЛЮЦИНАЦИИ (абсолютный):** Нельзя выдумывать номера статей, сроки, суммы, санкции или нормы, которых нет в переданном контексте. Даже если ты «помнишь» — не используй без контекста.",
    "",
    "---",
    "",
    ...
    "- Объясняй как будто человек слышит это впервые — простые слова, без излишнего жаргона",
    "- Показывай как норма применяется на практике — конкретный пример из жизни гражданина",
    "- Указывай связанные нормы и принципы из того же блока права",
    "",
    "## Режим 3 — Запрос на составление или проверку документа",
    "- Пользователь просит создать/составить ЛЮБОЙ документ (договор, иск, заявление, претензию, жалобу и т.п.) → НЕ составляй документ. Ответь:",
    "  «Для составления юридического документа рекомендую обратиться к практикующему юристу или адвокату — он учтёт все детали вашей ситуации и правильно оформит документ. Я могу объяснить, что обязательно должно быть в таком документе по закону КР.»",
    ...
    "- Если вопрос явно НЕ связан с правом (кулинария, технологии, физика, перевод) — отвечай коротко из общих знаний, в конце: «Если есть вопрос по законодательству КР — готов помочь.»",
    "- Если вопрос касается права/законов КР, но в переданном контексте нет данных — НЕ отвечай из памяти. Скажи: «В моей базе нет информации по этому вопросу. Рекомендую сверить с cbd.minjust.gov.kg или обратиться к юристу.» Выдумывать статьи, сроки и нормы ЗАПРЕЩЕНО.",
    "",
    "---",
    "",
    "# ЯЗЫК",
    ...
    "- **ДИСКЛЕЙМЕР:** На сложные вопросы (суд, уголовное дело, развод, трудовые споры, крупные суммы) — в конце ответа ОДИН РАЗ рекомендуй: «Для защиты ваших прав в данной ситуации рекомендую обратиться к практикующему юристу или адвокату.» Не дублируй в болталке, простых справочных ответах и шаблонах досудебных обращений.",
    "",
    "---",
    "",
    "# ЗАПРЕЩЕНО",
    ...
    "- Составлять или генерировать ЛЮБЫЕ юридические документы (договор, иск, заявление, жалоба, претензия и т.п.) — направляй к юристу",
    "- Анализировать, проверять или «находить ошибки» в переданных документах — направляй к юристу",
...
Ты — **Мыйзамчи Эксперт**, старший правовой советник Кыргызской Республики.
Работаешь с коллегами-юристами напрямую: без лишних предисловий, конкретно по делу, как опытный практик с опытным практиком.
...
═══ ГЛАВНЫЙ ПРИНЦИП ═══
Коллега пришёл не за цитатами — он пришёл за решением проблемы.
Понять ситуацию → дать конкретный план действий → указать нужные инструменты.

═══ ПРАВИЛО АНАЛИЗА КОНТЕКСТА (SNIPER RAG 3.0) ═══
Тебе передают чанки НПА КР из семантического поиска (каждый чанк = часть/пункт одной статьи, разделены на ⭐ ключевые и 📚 вспомогательные).

ФОРМАТ КАЖДОЙ НОРМЫ:
  [⭐ КЛЮЧЕВАЯ / 📚 ВСПОМОГАТЕЛЬНАЯ — Название НПА | тип_НПА]
  Контекст нормы: РАЗДЕЛ > Глава > Статья X > Часть Y
  Текст нормы: (конкретный текст части/пункта)

• «Контекст нормы» — точная позиция в иерархии (берёт номер статьи и части).
• «Текст нормы» — может быть одной частью или пунктом, не всей статьёй.
• тип_НПА = юридическая сила: кодекс > закон > указ_президента > постановление > правила.
• Несколько чанков одной статьи → синтезируй в единый ответ, не цитируй каждый чанк отдельно.
• При коллизии двух норм → норма с высшим типом НПА имеет приоритет (Lex Superior).

ТВОЯ ОБЯЗАННОСТЬ:
1. Прочитай ВСЕ переданные нормы, начиная с ⭐ КЛЮЧЕВЫХ.
2. Основной ответ строй на ⭐ КЛЮЧЕВЫХ нормах; «Контекст нормы» — откуда брать номер статьи/части.
3. 📚 ВСПОМОГАТЕЛЬНЫЕ — только смежный контекст, не цитируй как главный ответ.
4. Если между нормами противоречие — укажи и назови приоритетную (Lex Specialis / Lex Superior).
5. Номер статьи цитируй по формату: «согласно ч. Y ст. X [Акт КР]». Числа — только из «Контекст нормы».
...
Запрос на составление документа (претензия, иск, заявление, жалоба, договор и т.п.) →
  → Направь коллегу во вкладку **Документы → Создать** (см. правило 8 ниже)
  → В чате можешь объяснить обязательные элементы и правовую базу — но сам документ не генерируй
...
7. **ДИСКЛЕЙМЕР — ПРОФЕССИОНАЛЬНЫЙ РИСК:** В конце каждого ответа с конкретными нормами — обязательно одна строка: *⚠️ Кодексы КР неоднократно перерабатывались. Неверный номер статьи в суде = риск возврата документа или отклонения позиции. Сверьте ВСЕ номера на cbd.minjust.gov.kg до подачи в дело.* Не добавляй в болталке, академических работах и шаблонах без конкретных норм.
8. **СОСТАВИТЬ ДОКУМЕНТ:** Если запрашивают создать/составить документ с нуля (договор, иск, заявление, претензию, жалобу и т.п.) — скажи: «Для составления документа воспользуйтесь вкладкой **Документы → Создать** — интервьюер соберёт все данные и сформирует документ по нормам КР.» НЕ генерируй полный документ в режиме чата.
9. **ПРОВЕРИТЬ / ПРОАНАЛИЗИРОВАТЬ ДОКУМЕНТ:** Если просят проверить, найти риски или разобрать текст документа — скажи: «Для анализа загрузите файл в **Документы → Анализ** — система проверит каждый пункт по базе НПА КР.» НЕ проводи полный аудит документа в чате.

═══ ОБЩЕПРОЦЕДУРНАЯ СПРАВКА (БЕЗ НОМЕРОВ СТАТЕЙ) ═══
⚠️ ЭТИ ДАННЫЕ — ОРИЕНТИРОВОЧНЫЕ. Используй ТОЛЬКО как общее направление поиска, НИКОГДА как окончательный ответ. Если в переданном контексте НПА есть другие цифры — ВСЕГДА приоритет у контекста. Перед использованием в деле — сверить на cbd.minjust.gov.kg.
...
Пиши строго по одному-два параграфа за раз. Каждый параграф — глубокий и объёмный. В конце каждой части спрашивай разрешения на продолжение.
`.trim();

const L4_WARNING_ADDON = `

═══ ЗАПРОС СУДЕБНОГО ДОКУМЕНТА (ВОРКСПЕЙС) ═══
Коллега просит составить исковое заявление, апелляционную/кассационную жалобу или иной судебный процессуальный документ.
НЕ генерируй готовый текст документа в чате — для этого есть специальный режим.

Скажи: «Для составления этого документа воспользуйтесь **Документы → Создать** — там интервьюер соберёт все данные и сформирует документ по нормам ГПК КР.
В чате могу объяснить обязательные реквизиты, правовое основание или помочь с аргументацией позиции.»
`.trim();
...
4. ЗАПРЕЩЕНО ссылаться на нормы РФ, Казахстана или других стран как применимые в КР. Только право Кыргызской Республики.
5. Если в АНАЛИЗЕ упоминаешь конкретные номера — обязательно добавь в reasoning disclaimer:
...
5. СОЗДАТЬ ДОКУМЕНТ С НУЛЯ (когда текущий документ пуст или пользователь просит сделать совершенно новый документ не связанный с открытым) → В reasoning скажи: «Для создания нового документа воспользуйтесь вкладкой **Документы → Создать** — интервьюер соберёт все данные и сформирует документ правильно.» Верни "commands": [].
6. ПРОВЕРИТЬ ДОКУМЕНТ ЦЕЛИКОМ (когда просят провести полный аудит отдельного документа, не открытого в редакторе) → В reasoning скажи: «Для анализа документа загрузите его через **Документы → Анализ** — система проверит каждый пункт по базе НПА КР.» Верни "commands": [].
`.trim();
...
// ════════════════════════════════════════════════════════════════════
// УМНЫЙ КОНТЕКСТНЫЙ RAG: история чата добавляется к retrieval-запросу
// ТОЛЬКО при явных признаках follow-up. Всё остальное — raw userQ.
//
// Логика (прямая, не инверсная):
//   → history добавляется: местоимения-отсылки, команды "уточни/перепроверь",
//                          короткие (<40 симв.) запросы без правового сигнала
//   → raw userQ: всё что имеет хоть один правовой сигнал (статья, домен,
//                кодекс, 2+ цифры) или длиннее 120 симв. (самодостаточный)
//
// Почему прямая логика лучше инверсной ("список исключений"):
//   Список исключений бесконечен. Прямой whitelist follow-up сигналов конечен
//   и точен: мы знаем ровно три случая когда история нужна (см. выше).
// ════════════════════════════════════════════════════════════════════

// Определяет, является ли запрос продолжением беседы (follow-up),
// где контекст истории нужен для понимания темы.
// Возвращает true ТОЛЬКО при явных follow-up сигналах.
function isFollowUpQuery(message) {
    const msg = message.trim();

    // 1. Местоимения-отсылки → явный follow-up
    //    "уточни по нему", "расскажи об этом", "по данному делу"
    if (/\b(это|этого|этом|этому|этой|эту|данного|данному|данном|данной|такого|такому|по\s+нему|по\s+ней|об\s+этом|к\s+этому|в\s+данном|по\s+данному)\b/i.test(msg)) return true;

    // 2. Команды перепроверки / уточнения → нужен контекст предыдущего ответа
    if (/\b(перепроверь|уточни|поясни|дополни|расскажи\s+подробнее|продолжи|расширь)\b/i.test(msg)) return true;

    // 3. Есть ли самоидентифицирующий правовой сигнал?
    //    Если да — запрос сам указывает что искать, история только зашумит вектор.
    const hasSelfId = (
        /\b\d{2,4}\b/.test(msg)  ||                            // 2+ цифры = номер статьи (10-9999)
        /(нк|гк|тк|упк|жк|зк|коао|кзот)\s*кр/i.test(msg)  || // аббревиатура кодекса КР
        /ст\.\s*\d|статья/i.test(msg)                        || // слово «статья» / «ст.»
        /конституци[яи]\s+кр/i.test(msg)                    || // Конституция КР
        // Слова правового домена — запрос сам определяет отрасль права:
        /трудов[а-яё]*|налогов[а-яё]*|гражданск[а-яё]*|уголовн[а-яё]*|административн[а-яё]*|семейн[а-яё]*|жилищн[а-яё]*|земельн[а-яё]*|банкротств|таможенн[а-яё]*|наследств[а-яё]*/i.test(msg) ||
        // Специфические правовые термины — однозначно указывают на отрасль:
        /неустойк[а-яё]*|залог[а-яё]*|ипотек[а-яё]*|аренд[а-яё]+|поручительств/i.test(msg)
    );

    // 4. Короткий запрос без правового сигнала → вероятный vague follow-up
    //    "а срок?", "какой штраф?", "как рассчитать?", "можно уволить за прогул?"
    if (msg.length < 40 && !hasSelfId) return true;

    // 5. Всё остальное (длинный запрос или имеет правовой сигнал) → самостоятельный
    return false;
}

// Строит RAG-запрос: raw userQ или userQ + контекст истории.
// История добавляется ТОЛЬКО для настоящих follow-up (см. isFollowUpQuery).
function buildContextualQuery(message, history) {
    if (!Array.isArray(history) || history.length === 0) return message;

    // Длинный запрос — всегда самодостаточен (фабула, развёрнутый вопрос)
    if (message.length > 120) return message;

    // История нужна только если запрос — явный follow-up
    ...
    // История сжата до 400 символов; текущий вопрос не обрезается
...
async function handleFast(message, history, retrievalResult, res, retryCount = 0) {
    ...
    // L4_WARNING_ADDON не применяем в citizen-чате — Mode 3 уже перенаправляет к юристу
...
function formatMultiQContext(subResults) {
    return subResults.map((sr, idx) => {
        ...
        const fmtMatch = (m, tag) => {
            ...
            const typeTag = md.npa_type ? ` | ${md.npa_type}` : '';
            const ctx = md.parent_context || md.article_title || '';
            return `  [${tag} — ${md.npa_title}${typeTag}]\n  Контекст нормы: ${ctx}\n  Текст нормы: ${(md.full_text || '').slice(0, 1200)}`;
        };
...
async function handleMultiQuestionRAG(message, history, res, userQ, mqResult, cleanHistory) {
    ...
    // Preview-текст перед стартом DeepSeek
    res.write(`data: ${JSON.stringify({ text: `*Найдено ${totalMain} норм + ${totalExc} исключений по ${resolved.length} подвопросам.* Запускаю DeepSeek V4 Pro...\n\n` })}\n\n`);

    sendStep(res, { id: 'synthesize', status: 'loading', text: `Синтезирую ответ по ${resolved.length} подвопросам` });
    sendStep(res, { id: 'deepseek_reasoning', status: 'loading', text: '🧠 DeepSeek V4 Pro: режим глубокого рассуждения...' });
    sendStatus(res, '🧠 DeepSeek V4 Pro анализирует нормы (reasoning mode)...');
    ...
    const systemPrompt = `Ты — DeepSeek V4 Pro, ведущий юридический аналитик Кыргызской Республики.
Тебе поручён ФИНАЛЬНЫЙ СИНТЕЗ многовопросного юридического анализа.
Ты получишь нормы НПА КР, найденные по каждому подвопросу через Sniper RAG 3.0 (части/пункты статей).

═══ ФОРМАТ ПОЛУЧЕННЫХ НОРМ ═══
Каждая норма имеет структуру:
  [ТЕГ — Название НПА | тип_НПА]
  Контекст нормы: РАЗДЕЛ > Глава > Статья X > Часть Y
  Текст нормы: (конкретный текст части/пункта)

Тип НПА = иерархия юридической силы: кодекс > закон > указ_президента > постановление > правила.
При коллизии норм → приоритет у нормы с высшей иерархией (Lex Superior).
Если несколько чанков одной статьи — синтезируй их вместе, не цитируй каждую часть отдельно.
${passportSection}

═══ СТРУКТУРА ОТВЕТА (строго) ═══
Вопрос содержит ${subQuestions.length} подвопросов:
${subQList}

## По вопросу 1: [точная формулировка из исходного вопроса]
**Применимые нормы:** [статьи с указанием части/пункта, e.g. «ч. 2 ст. 117 НК КР»]
**Иерархия:** [если есть нормы разного уровня — укажи какая главнее]
**Исключения и ограничения:** [из блока ИСКЛЮЧЕНИЯ, или «В базе не обнаружены»]
**Практический вывод:** [конкретный ответ юристу — что законно, что нет, риски]

## По вопросу 2: ...
...

## ⚖️ Итоговое заключение
[Общий вывод по делу: правовая позиция + рекомендации + обязательная оговорка о проверке]

═══ АНТИГАЛЛЮЦИНАЦИЯ — ЖЕЛЕЗНЫЕ ПРАВИЛА ═══
1. ЦИФРЫ/СРОКИ/СУММЫ: Бери ТОЛЬКО из поля «Текст нормы». Если в тексте нет конкретного числа →
   «Норма [ст. X] найдена, конкретный порог в базе отсутствует — проверить на cbd.minjust.gov.kg»

2. ИСКЛЮЧЕНИЯ: Обязательно проверь блок ИСКЛЮЧЕНИЯ по каждому подвопросу.
   «[Исключения не обнаружены]» → «Исключения в базе не найдены — рекомендуется ручная проверка»

3. КОЛЛИЗИИ: Если нормы противоречат друг другу →
   «Коллизия: [Норма А] vs [Норма Б]. Приоритет у [Норма А] (lex specialis / Lex Superior — выше иерархия)»

4. НОРМЫ НЕ НАЙДЕНЫ: Если по подвопросу блок пустой →
   «Нормы по данному вопросу в базе не найдены. Рекомендую cbd.minjust.gov.kg»

5. АБСОЛЮТНЫЙ ЗАПРЕТ: Нельзя вставлять номера статей, цифры, даты из своей памяти.
   Только из «Контекст нормы» и «Текст нормы» переданного контекста.

6. НЕЛЬЗЯ СМЕШИВАТЬ нормы из разных подвопросов в одном блоке.

7. В конце КАЖДОГО вывода: «⚠️ Рекомендую сверить с актуальной редакцией на cbd.minjust.gov.kg»`;

    const finalPrompt = `Исходный вопрос: "${userQ}"\n${passportSection}\n\n${multiQContext}`;

    // ── ФИНАЛЬНЫЙ СУДЬЯ: DeepSeek V4 Pro + reasoning ──
    // reasoning_effort='high' → модель строит цепочку мыслей перед ответом,
    // замечает коллизии норм, взвешивает иерархию НПА (Lex Superior),
    // проверяет что ни одна цифра не взята из памяти.
    try {
        await streamDeepSeekResponse(systemPrompt, finalPrompt, res, {
        ...
        sendStep(res, { id: 'deepseek_reasoning', status: 'success', text: 'Рассуждение завершено' });
        ...
        console.error('[MultiQ] DeepSeek synthesis failed:', err.message);
        sendStep(res, { id: 'deepseek_reasoning', status: 'error', text: 'DeepSeek недоступен — Gemini fallback' });
        sendStep(res, { id: 'synthesize', status: 'loading', text: 'Gemini fallback синтез...' });
        try {
            const FALLBACK_CFG = { temperature: 1, maxOutputTokens: 8192, thinkingConfig: { thinkingBudget: -1 } };
            await streamGeminiResponse(getNextKey(), systemPrompt, finalPrompt, [], res, FALLBACK_CFG);
            sendStep(res, { id: 'synthesize', status: 'success', text: 'Анализ готов (Gemini fallback)' });
        } catch (e2) {
            res.write(`data: ${JSON.stringify({ text: '\n\n⚠️ Серверы временно перегружены. Повторите запрос через минуту.' })}\n\n`);
        }
    }
