...
ПРАВИЛА: факты бери ТОЛЬКО из диалога, не выдумывай имена/суммы/даты/адреса (нет → ''). Запросы пиши развёрнуто, называя конкретные институты, кодексы и законы КР, чтобы векторный поиск нашёл максимум норм. Обязательно укажи СПЕЦИАЛЬНЫЙ закон/кодекс, профильный для предмета спора.`;
  ...
  const DRAFTER_SYS = (tpl, titleWord) => {
    // ── ДВУСТОРОННИЙ ДОКУМЕНТ (договор) — иная структура: преамбула, разделы,
    //    нумерованные пункты, реквизиты и подписи двух сторон. ──
    if (tpl.bilateral) {
...
- paragraph (по ширине): строка места и даты «г. ____, «__» ____ 20__ года»; ПРЕАМБУЛА (стороны, как именуются, в лице кого, основание полномочий, «… договорились о нижеследующем:»)
- section_heading (bold, слева): нумерованный раздел «N. НАЗВАНИЕ РАЗДЕЛА»
- clause (по ширине): нумерованный пункт «N.N. …» (это обычный пункт раздела)
- requisites_table: финальный блок реквизитов/подписей В ДВЕ КОЛОНКИ. Формат: { "kind":"requisites_table", "left":"<Сторона 1: НАЗВАНИЕ\\nреквизиты\\n_______ /ФИО/>", "right":"<Сторона 2 аналогично>" }. Внутри left/right строки разделяй \\n; первая строка — название стороны
- spacer (runs:[])

СТРУКТУРА (адаптируй разделы под ВИД договора — лишние убери, нужные добавь):
${(tpl.structureHint || []).map((s, i) => `${i + 1}. ${s}`).join('\n')}

КАК СОСТАВЛЯТЬ (договор должен быть полным и рабочим, а не из трёх пунктов):
• Преамбула: точно укажи обе стороны из досье, как они именуются («Заказчик»/«Исполнитель» и т.п.), в лице кого и на каком основании действуют.
• Раскрой КАЖДЫЙ раздел нумерованными пунктами (1.1, 1.2, …; 2.1, …). Предмет, цена и порядок расчётов, права и обязанности, ответственность, срок, разрешение споров, заключительные положения.
• Привязывай условия к нормам — по релевантным пунктам давай ссылку на статью ГК КР отдельным run (italic:true, cite). Цитируй ТОЛЬКО статьи из ЭТАЛОННОГО списка (RAG); не выдумывай номера.
• В конце — section_heading «РЕКВИЗИТЫ И ПОДПИСИ СТОРОН», затем ОДИН блок requisites_table с left (Сторона 1) и right (Сторона 2): наименование/ФИО, адрес, реквизиты и строка подписи «_______ /ФИО/».

ЖЕЛЕЗНЫЕ ПРАВИЛА:
1. НЕ выдумывай факты (наименования, ФИО, суммы, даты, реквизиты). Только из досье. Нет обязательной детали — ставь «____________».
2. Цитируй статьи ТОЛЬКО из эталонного списка норм (RAG), отдельным run с italic:true и cite. Следи за пробелами между run.
3. Используй РЕАЛЬНЫЕ применимые нормы ГК по виду договора — не ограничивайся общими фразами.
4. Верни ТОЛЬКО JSON-массив блоков. Первый символ ответа — «[», последний — «]».`;
    }

    const dw = tpl.demandWord || '';            // '' → у документа нет блока «Прошу/Требую»
...
ТИПЫ БЛОКОВ (kind):
- court (только для судебного документа: наименование суда) · party_header (стороны/адресат/заявитель — по строке на блок, справа) · spacer (runs:[]) · title (НАЗВАНИЕ заглавными, bold, центр${ttl ? `: «${ttl}»` : ''}) · subtitle («о …», центр)
...
• Сначала ФАБУЛА: подробно изложи обстоятельства из досье (стороны, даты, предмет, суть).
• Затем НЕСКОЛЬКО абзацев правового обоснования — задействуй ВСЕ применимые нормы из эталона:
  – сперва ТОЧНЫЕ нормы (прямое основание) — процитируй и объясни, как норма применяется к фактам;
...
${tpl.courtDoc
  ? `• Отрази ПРОЦЕССУАЛЬНЫЕ моменты со ссылками на ГПК КР: подсудность (почему этот суд), соблюдение срока исковой давности, размер и расчёт государственной пошлины.
• Отдельный абзац процессуального основания: «Руководствуясь ст. … ГПК КР, прошу:» со ссылками на процессуальные нормы.
• Каждое требование должно опираться на обоснование; для имущественного иска добавь требование о взыскании госпошлины.`
  : `• Отрази ПОРЯДОК и СРОКИ из группы «Порядок и сроки», если применимо к этому документу.
• НЕ добавляй блок суда (court), госпошлину, подсудность и ссылки на ГПК — это НЕ судебный документ. Адресат/заявитель — блоками party_header (справа).${dw ? `\n• Каждое требование в «${dw}:» формулируй чётко и конкретно (что и в какой срок).` : ''}`}
  ...
  router.post('/draft-document', async (req, res) => {
    ...
    try {
      ...
      const normalizeBlock = (b) => ({
        ...
        ...((b && b.kind) === 'requisites_table' ? { left: stripCjk(String((b && b.left) || '')), right: stripCjk(String((b && b.right) || '')) } : {}),
