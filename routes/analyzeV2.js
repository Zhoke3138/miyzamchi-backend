...
ЖЕЛЕЗНЫЕ ПРАВИЛА:
1. Если ЭТАЛОННЫЙ СПИСОК содержит нормы — задействуй ВСЕ применимые, не ограничивайся одной-двумя. Если список пуст или помечен «Векторная база не вернула норм» — ОБЯЗАТЕЛЬНО применяй свои профессиональные знания ГК КР, ГПК КР и профильных законов КР: называй конкретные статьи, объясняй их применение к фактам дела. В любом случае НЕ придумывай несуществующие статьи — только реально существующие нормы КР.
...
5. Верни ТОЛЬКО JSON-массив блоков. Первый символ ответа — «[», последний — «]».`;
  ...
  router.post('/draft-document', async (req, res) => {
    ...
    try {
      ...
      const refBlock = refParts.length ? refParts.join('\n\n') : `(Векторная база не вернула норм по данному запросу. ОБЯЗАТЕЛЬНО применяй свои профессиональные знания НПА Кыргызской Республики — цитируй конкретные статьи ГК КР, ГПК КР и профильных законов КР, которые применимы к существу дела. Каждую ссылку на норму оформляй отдельным run с italic:true и cite:"ст. XXX ГК КР". Не придумывай несуществующие статьи — только реально существующие нормы КР.)`;
