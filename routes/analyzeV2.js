...
ТИПЫ БЛОКОВ (kind):
- court (только для иска: наименование суда) · party_header (стороны/адресат/заявитель — по строке на блок, справа) · spacer (runs:[]) · title (НАЗВАНИЕ заглавными, bold, центр: «${tpl.titleWord || tpl.label}») · subtitle («о …», центр)
- paragraph (фабула / правовое обоснование) · demand_heading («${tpl.demandWord || 'Прошу'}:») · demand_item (нумерованное требование/просьба)
...
• Каждое требование в «Прошу:» должно опираться на обоснование; для имущественного иска добавь требование о взыскании госпошлины.`
  : `• Отрази ПОРЯДОК и СРОКИ из группы «Порядок и сроки» (для претензии — досудебный порядок и срок добровольного удовлетворения с предупреждением об обращении в суд; для заявления — порядок и срок рассмотрения обращения).
• НЕ добавляй блок суда (court), госпошлину, подсудность и ссылки на ГПК — это НЕ судебный документ. Адресат и заявитель — блоками party_header (справа).
• Каждое требование в «${tpl.demandWord || 'Прошу'}:» формулируй чётко и конкретно (что и в какой срок).`}
