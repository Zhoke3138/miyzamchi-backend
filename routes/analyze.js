...
Документ можно использовать в его текущей редакции. Перед подписанием рекомендуется сверить актуальность норм по [cbd.minjust.gov.kg](https://cbd.minjust.gov.kg).`;
        ...
        // 2026-05-31: повысили reasoning_effort. Судья теперь сравнивает
        // оригинальный текст с отчётом агентов — это требует "подумать",
        // а не просто пересказать. Heavy → high, fast → medium.
        const judgeReasoning = isHeavyPath ? 'high' : 'medium';
...
1) 🛑 ЗАПРЕТ НА УПОМИНАНИЕ ТЕХНИЧЕСКИХ НОМЕРОВ (ЧАНКОВ).
ВНИМАНИЕ! Отчёт агентов разбит на "пункты" (п.1, п.5, п.13 и т.д.). Знай: это ИСКУССТВЕННАЯ техническая нарезка алгоритма! В оригинальном документе пользователя (он предоставлен тебе ниже в секции "ОРИГИНАЛЬНЫЙ ТЕКСТ ДОКУМЕНТА") этих пунктов МОЖЕТ НЕ БЫТЬ.

⛔ КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать в резюме фразы вроде:
   • "В пункте 13 ошибка"
   • "В п.10 указано"
   • "Фрагмент 25 содержит"
   • "Пункт 8 — это просто заголовок"
   • "В пункте 7 общие фразы"
   • "Недостаточно информации в пункте N"
   • "Пункт обрывается"
Пользователь не поймёт, о чём речь — у него в документе нет таких номеров.

✅ ВМЕСТО ЭТОГО: Опирайся на ОРИГИНАЛЬНЫЙ ТЕКСТ ДОКУМЕНТА. Ссылайся на СУТЬ текста, разделы или смысловые блоки:
   • "В разделе о нарушениях Конституции указана..."
   • "При описании обстоятельств дела автор ссылается на..."
   • "В части про ответственность сторон сказано..."
   • "Во вводной части документа отмечается..."

✅ Ссылаться на реальные номера пунктов можно ТОЛЬКО если сам автор явно пронумеровал их в оригинальном тексте (например, "пункт 3.1 Договора", "статья 5", "раздел II.3"). Сверяйся с оригиналом перед тем как назвать любой номер.
...
3) 🛑 ЗАПРЕТ НА УГАДАЙКУ НОМЕРОВ СТАТЕЙ.
Если в вердиктах агентов отмечено, что пользователь сослался на неверную статью (например, перепутал номер в Конституции / ГК / УК), ты НЕ ИМЕЕШЬ ПРАВА называть "правильный" номер из своих знаний.

Причина: законы Кыргызской Республики регулярно меняются (Конституция 2010 → 2021, новые редакции кодексов). В своих весах ты держишь СТАРЫЕ номера. Любой номер из памяти = галлюцинация и подрыв доверия юриста к платформе.

⛔ ЗАПРЕЩЕНО писать:
   • "Правильная норма — статья 26"
   • "На самом деле это статья 137"
   • "Корректный номер — статья 234"
если этот номер не был явно подтверждён выдачей search_legislation_kg в вердиктах агентов (поле applicable_articles).

✅ Если корректный номер ЕСТЬ в applicable_articles ниже — можешь его процитировать со ссылкой "согласно ст. N такого-то кодекса".

✅ Если корректного номера НЕТ в applicable_articles — формулируй мягко:
   "Указанная норма регулирует другой вопрос. Рекомендуется уточнить актуальный номер статьи по официальному источнику (cbd.minjust.gov.kg)."
...
[ТВОЯ ЗАДАЧА]
Сформируй итоговое заключение (Executive Summary), сверяя отчёт агентов с ОРИГИНАЛЬНЫМ ТЕКСТОМ выше. Когда упоминаешь риск — указывай СМЫСЛОВОЙ БЛОК оригинала, а не номер чанка из отчёта.

Структура ответа (markdown):
1. **Краткий вывод** — 2-3 предложения о документе в целом.
2. **Ключевые риски** — выдели 2-3 самых серьёзных риска. Каждый риск привяжи к смысловому блоку оригинала ("в части про X", "в разделе о Y"), а НЕ к "пункту N" из отчёта.
