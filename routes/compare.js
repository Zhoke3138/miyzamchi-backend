...
const COMPARE_SEG_LIMIT   = 90;       // лимит сегментов НА ДОКУМЕНТ (regex дешёвый, не жалеем; договоры бывают 50-80 пунктов)
...
// ── ПРОМПТ СУДЬИ (Reduce) ────────────────────────────────────────────
function makeJudgePrompt(side) {
  const sideLine = side === 'theirs'
    ? 'Ты анализируешь с позиции ДРУГОЙ стороны (контрагента). Раскрой, что она выиграла от изменений и какие риски принимает наша сторона.'
    : side === 'neutral'
    ? 'Ты анализируешь НЕЙТРАЛЬНО: покажи выгоды и риски обеих сторон равномерно.'
    : 'Ты анализируешь с позиции НАШЕЙ стороны (клиента, предоставившего редакцию). Первоочередная задача — защитить её интересы.';
  return `Ты — **Старший партнёр** юридической фирмы КР. ${sideLine}
...
Сформируй Executive Summary для клиента строго по четырём секциям (markdown):
...
На основании ПЕРЕДАННОЙ СВОДКИ изменений выяви: какие типичные для данного типа документа
условия НЕ обсуждались ни в одной редакции — то есть остались вне переговоров. Примеры:
форс-мажор, порядок уведомлений, применимое право, ограничение ответственности, конфиденциальность,
право на односторонний отказ. Напиши ТОЛЬКО то, что реально отсутствует в сводке (без домыслов).
Если всё покрыто — «Все ключевые условия присутствуют в обеих редакциях.»
...
module.exports = function registerCompareRoute(deps) {
    const {
        ...
        // segmentDocument (LLM) НЕ используем в compare: он недетерминирован и ломает ALIGN.
        // Сегментация — детерминированная regex (segmentForCompare из compareService).
    ...
    const required = { app, getEmbedding, generateContentResilient, streamDeepSeekResponse,
        extractDocumentContext, formatDocContext,
...
Верни ровно ${pairs.length} результат(ов) в порядке idx по формату из системной инструкции.`;
    ...
    router.post('/api/compare-documents', compareLimiter, requireClientToken, async (req, res) => {
        return requestTelemetry.run({ res, label: 'compare-documents' }, async () => {
            try {
                const { oldDocumentText = '', newDocumentText = '', side = 'ours' } = req.body || {};
                const validSide = ['ours', 'theirs', 'neutral'].includes(side) ? side : 'ours';
                ...
                // ДЕТЕРМИНИРОВАННАЯ сегментация (regex, БЕЗ LLM): два почти одинаковых
                // документа режутся ОДИНАКОВО → одинаковое число сегментов → ALIGN не
                // съезжает и не плодит ложные изменения. LLM-сегментатор (segmentDocument)
                // здесь намеренно НЕ используется — он недетерминирован (давал 49 vs 53).
                const oldSegsRaw = segmentForCompare(oldDocumentText, COMPARE_SEG_LIMIT);
                const newSegsRaw = segmentForCompare(newDocumentText, COMPARE_SEG_LIMIT);
...
нарушения НПА дай готовую формулировку-контраргумент со ссылкой на указанную норму.`;
                ...
                try {
                    ...
                    await streamDeepSeekResponse(makeJudgePrompt(validSide), judgeUser, res, {
                        temperature: 0.25,
