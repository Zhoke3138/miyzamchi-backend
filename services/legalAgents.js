...
ПРАВИЛА:
- skip: верни true ТОЛЬКО если фрагмент ЦЕЛИКОМ состоит из нейтрального контента —
  имена сторон и должности, реквизиты (адрес, ИНН, ОГРН, банк, р/с, тел.), даты, подписи,
  печать, вводные слова («настоящим уведомляем», «с уважением»), суммы без правового контекста.
  Верни false если есть хоть одна правовая норма, ссылка на закон, обязательство или требование.
- npa: ТОЛЬКО если в тексте ЯВНО написано название или аббревиатура НПА — «ГК КР», «ст. 57 ТК КР»,
  «Гражданский кодекс», «Конституция», «Закон о ...» и т.п. Раскрывай аббревиатуры
  (ГК КР → Гражданский кодекс Кыргызской Республики).
  НЕЛЬЗЯ выводить НПА по косвенным признакам: тип юр. лица (ОсОО, АО) ≠ Закон об ООО;
  вид договора ≠ ссылка на ГК; должность ≠ Трудовой кодекс.
  Если явного названия/аббревиатуры НПА нет — верни null.
- article: номер статьи, если он назван в тексте цифрой (например "57"), иначе null.
- queries: 2-4 запроса — юридические формулировки сути фрагмента для поиска по базе НПА.
  Если npa определён (явная ссылка!), включай его название в запросы.
- Без markdown — только JSON.`;

async function expandQuery(chunkText) {
  const fallback = { skip: false, npa: null, article: null, queries: [String(chunkText || '').slice(0, 1500)] };
  try {
    const raw = await clients.geminiJson({
...
ПРАВИЛА:
- Единый заголовок раздела с ошибками: "## Анализ: <doc_header>" где doc_header берёшь из поля
  "doc_header" переданных данных. Если doc_header пуст — "## Анализ вашего документа".
  НЕ создавай отдельные ## заголовки для каждого НПА.
  Внутри раздела группируй ошибки по НПА жирным подзаголовком "**Нарушения <НПА>:**".
- Для каждой ошибки: 🔴 ОШИБКА, КОРОТКИЙ заголовок (до 5-7 слов) жирным, суть и как исправить.
...
- Блок blind_spots — это НЕ ошибки. Вынеси их ОТДЕЛЬНОЙ секцией в конце
  "## ⚠️ Требуется ручная проверка": перечисли акт и пометь, что его нет в базе.
  НИКОГДА не помечай blind_spots как 🔴 ОШИБКА.
- ОБЯЗАТЕЛЬНАЯ финальная секция "## ✅ Применимые нормы (проверено)" (данные из confirmed) —
  показывает юристу какие нормы подтверждены как соответствующие тексту документа.
  Формат каждого пункта — СНАЧАЛА ЧТО из документа, ПОТОМ КАКОМУ закону соответствует:
  • "- Ваше утверждение о [суть thesis, до 8 слов] соответствует [npa], ст. [article]."
    Пример: "- Ваше утверждение об ограничении выезда за границу соответствует Закону КР «О статусе судебных исполнителей», ст. 52."
  • Если article отсутствует: "- Ваше утверждение о [суть thesis] соответствует [npa]."
  • Если npa и article оба отсутствуют: используй cited_articles[0] или "[суть thesis] — соответствует закону КР."
...
async function judge({ graph, effort, model = 'deepseek-v4-pro', thinking = 'enabled', onDelta = null, state = null }) {
  const { groups, blind_spots, confirmed } = groupByNpa(graph);

  // Краткое описание документа из extractGlossary (поле header).
  // Пример: "Досудебная претензия ОсОО «Бишкектеплосервис» о взыскании задолженности за тепло."
  const doc_header = (state && state.header) ? String(state.header).slice(0, 200) : '';
  ...
  const userPrompt = `doc_header: "${doc_header || 'вашего документа'}"\n\nСТРУКТУРИРОВАННЫЙ ОТЧЁТ (errors сгруппированы по НПА, плюс blind_spots и confirmed):\n${JSON.stringify({ groups, blind_spots, confirmed }, null, 2)}\n\nСформируй финальный отчёт: раздел ошибок, ручная проверка, подтверждённые нормы.`;
